...
- 建立共享、可序列化、可校验和可稳定排序的 EntryQuery/EntryFilter 模型。
- 将现有 GET 查询参数归一化到该模型，而不是同时维护两套查询语义。
- 查询 descriptor、cursor digest 和缓存键使用同一规范化结果。
- 先支持 `part`、`tag`、`source`、`derivedFrom` 和本节列出的简单 presence/count/date 条件。
