...
| 当前搜索命中字段 | 当前查询在指定字段至少命中一次 | 直接更新列表运行期 `search.fields/fuzzyFields` | 不再创建高级筛选；搜索文本和严格/fuzzy 语义保持不变，词典级字段设置只作为新会话默认值，分析 slice 缓存身份包含运行期搜索签名。 |

除“当前搜索命中字段”只调整运行期搜索范围外，以上结构项目应优先进入统一 EntryQuery。它们不需要由数据分析页面预先计算完整 ID 数组。
...
- 进入高级筛选、结构事实失效或词条写入后，前端通过批量 `/entries/filter-facts` 自动补齐未知事实。搜索输入只重查当前 `Filter ∩ Search`，不会为其他变体重复 strict/fuzzy 探测；手动刷新继续强制重新读取当前窗口并重新验证结构事实。
...
- 高级筛选状态保存结构 descriptor、搜索字段范围、视图级搜索文本、当前循环变体和显示元数据，不保存完整匹配 ID。
...
- 刷新只使当前 entries 查询及其定位页面的前端缓存失效，保留同词典的其他查询、词根窗口和 facets；结构事实由 filter-only 批量请求独立重新验证。正常搜索、循环和有效的非当前响应会继续沉淀到查询页缓存或结构事实缓存。feature result 型筛选继续沿用原有刷新逻辑。
