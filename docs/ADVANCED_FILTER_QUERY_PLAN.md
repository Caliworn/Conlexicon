...
F0 时，进入高级筛选后前端会以 `entryIds` 扫描当前完整词典快照。该实现曾绕过 `/entries` 查询会话、窗口读取和目标定位；除标签与质量问题外，多数筛选刷新时也只会移除已经不存在的词条 ID。F3 已迁移其中可稳定表达为 SQL 的条件，剩余 `entryIds` 仅属于尚待 F4/F5 接管的 feature result。
...
- 查询型循环变体只保存 descriptor 与可用状态；刷新会重新请求当前窗口，并以每个变体一条 `limit=1` 查询更新循环按钮可用性。
- IPA 自动生成、Gloss、形态和质量问题仍保存前端功能结果 ID 与可选问题详情。这是 F4/F5 尚未迁移的明确边界，不是普通 filter 的兜底。
...
- 刷新会使前端窗口缓存失效并重新读取真实结果，同时探测循环变体是否仍有匹配；feature result 型筛选继续沿用原有刷新逻辑。
