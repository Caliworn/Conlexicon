...
- `part`、原始精确标签、来源文本和既有 `derivedFrom` 已进入统一 filter；presence、sourceCount 和 UTC activity day 也具有严格 descriptor 与冲突校验。
- `derivedFrom` 的模型预留 `{ entryId, reference }`，当前 GET 参数映射到 `reference`；后续来源 ID 化不应把 `source_key` 暴露为正式 descriptor 身份。

### F2：普通筛选接线（已完成）

- repository 已将词性、标签、来源文本、衍生关系、字段存在性、来源数量和 UTC 活动日期编译为稳定 SQLite 条件。
...
- 本阶段只完成查询模型、transport 和 repository 接线。现有高级筛选 UI 仍保存完整 ID 数组；标签、覆盖率、衍生词、多来源、日期和当前搜索字段入口迁移到 descriptor 属于 F3。
...
- 正向和负向字段存在性、来源数量区间、UTC 创建/修改日期、组合标签条件和基于词条 ID 的衍生关系均由 SQLite 契约测试覆盖。
