...
- 高级筛选、数据分析和质量检查尚未接入同一查询边界；关系类详情与词汇网络已迁移，不再属于该缺口。
...
- 普通编辑器列表与详情已切到上述边界；高级筛选、数据分析和质量检查仍会在 Q5 使用完整活动词典 snapshot。详情来源、完整编辑衍生词和词汇网络等关系消费者已经迁移到共享关系 API，不再列入剩余本地消费者。这是尚未迁移模块的当前实现，不是 summary/detail 的运行时兜底。
