...
产品前端与 repository 公共查询上限统一为普通词条 200 条、父级词根组 100 组，并最多保留 5 个已加载数据窗口。`QueryPageCache` 可以缓存这些窗口的 summary DTO，当前上限为 4 项、约 16 MiB，并按估算字节和 LRU 双重限制；单条完整详情使用独立的 12 项、约 12 MiB 缓存。基准/诊断使用同一窗口，通过会话构建统计与 cursor 分离测量匹配和响应成本。
...
- 产品前端与 repository 的公共硬上限统一为普通词条 200 条、父级词根组 100 组；超限请求明确失败。旧 `limit=10000/2000` 基准旁路已删除，诊断改用同一产品窗口和会话 `buildMs`、命中/构建统计。
