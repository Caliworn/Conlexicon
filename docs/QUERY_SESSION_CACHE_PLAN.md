...
- 词条切换已经改为局部提交；详情来源、详情/完整编辑衍生词和词汇网络也已共用后端关系 API 与前端关系缓存。
- 普通词条和词根父级窗口定位接口已经接入前端自动滚动；未加载目标不再通过完整活动词典 snapshot 推算页号。

当前缺口：

- 稳定 SQL 高级筛选已经接入同一查询边界；数据分析、质量检查以及 IPA/Gloss/形态等功能结果集尚未建立服务端 result session。关系类详情与词汇网络已迁移，不再属于该缺口。
...
2. 为纯滚动窗口化提供稳定、不可解析且绑定查询版本的 cursor。
...
前端缓存 API 返回的可渲染窗口状态，而不是完整词典或完整词条详情：
...
产品前端目前每次读取普通词条 200 条或父级词根组 100 组，并最多保留 5 个已加载数据窗口。`QueryPageCache` 可以缓存这些窗口的 summary DTO，当前上限为 4 项、约 16 MiB，并按估算字节和 LRU 双重限制；单条完整详情使用独立的 12 项、约 12 MiB 缓存。显式基准/诊断仍可请求 repository 的大页上限，但该路径不进入正常 UI 缓存设计。
...
后端当前值为每词典 8 个会话、全局约 64 MiB、2 分钟 idle TTL；前端查询页缓存当前为 4 项、约 16 MiB，单条详情缓存为 12 项、约 12 MiB。这些是便于 10k 压测的实现参数，不是 API 语义，最终应由 `estimatedBytes` 和实际命中率调整。缓存不得因一个超大查询突破全局上限；单项超过上限时直接不缓存，但仍正常返回结果。TTL/LRU 只影响性能，不能改变 cursor 的可用性或查询结果。

## 9. 词根模式的额外边界

`/root-groups` 的 `limit` 只计算父级词根组数量。折叠组已只返回 root summary、衍生词数量和匹配状态；展开时通过 `/root-groups/:rootId/entries` 一次读取整组衍生词。子端点不接受 cursor/window/limit，也没有单组 2000 条硬上限；项目已明确不为现实中很小的单个词根家族维护第二层窗口。

“全部展开”使用全局状态意图，不要求所有父级窗口同时驻留；父级首窗返回每窗 `{ groupCount, derivedCount }` 轻量统计，用于未加载区域的展开高度估算，组内内容进入可见范围后才按组读取。`all` 模式中的单组收起是例外状态，父级窗口淘汰只释放 DTO，不清除展开意图。
...
9. 折叠组响应不携带子项；展开组一次读取整组，父级窗口淘汰不丢失全局展开/单组例外状态。
...
- 普通编辑器列表与详情已切到上述边界；F3 又将稳定 SQL 高级筛选接入同一窗口与查询会话。F4a 已让数据分析总览的轻量统计改用 `/analysis/query` widget DTO；F4b-1 已让 IPA 自动生成比较消费 feature query/location 和同一列表窗口。其他分析子页、质量检查以及 Gloss/形态 feature result 仍会使用完整活动词典 snapshot，等待 F4b-2/F4b-3/F5 迁移。详情来源、完整编辑衍生词和词汇网络等关系消费者已经迁移到共享关系 API，不再列入剩余本地消费者。这是尚未迁移模块的当前实现，不是 summary/detail 的运行时兜底。
...
单条 SQL 读取本身是主键及 `entry_id` 索引路径。Q4 已将列表重建从全部 summary DTO 收窄为“页占位 + 最多 5 个已加载窗口”，随后词条切换也已移除全局 `render()`：现在只同步已渲染卡片的选中态、详情、词汇网络状态和必要的高度/自动滚动，不再重新生成查询窗口的 DOM、key、高度估算与 offsets。详情来源、详情/完整编辑衍生词和词汇网络进一步共用 `/entry-relations/:entryId` 与同一前端缓存，不再从完整活动词典重建关系索引。

最终实装状态：

1. Q4 数据窗口化已经完成，列表状态、cursor、窗口拼接和滚动锚点已成为局部渲染边界。
...
3. 词条切换局部提交已经完成：只更新待选/选中卡片、详情、词汇网络状态和必要的滚动状态，不重建未变化的列表窗口。
4. 详情中的来源解析和衍生词已经改用共享关系 API/缓存；静态详情先显示，关系响应完成后局部补齐来源和衍生词区域。未缓存词条切换的主要等待现在是单条详情请求与一条关系请求，不再包含前端全词典关系索引重建。
