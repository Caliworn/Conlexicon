...
- Classic, Layered Glass, and Liquid Glass skins selected from an extensible skin menu, dark mode, and Chinese/English UI switching, with the global interface skin, theme, and language remembered in `data/index.json`. Liquid Glass currently provides its token-based material baseline; SVG refraction and dynamic caustics remain staged enhancements.
...
- 通过可扩展皮肤菜单选择经典、层叠玻璃或液态玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。液态玻璃当前完成 token 材质基线，SVG 折射和动态焦散仍属于后续阶段。
