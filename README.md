...
- Classic, Layered Glass, and Liquid Glass skins selected from an extensible skin menu, dark mode, and Chinese/English UI switching, with the global interface skin, theme, and language remembered in `data/index.json`. Layered Glass treats the mutually exclusive entry display and primary entry form as one focused workspace shell while keeping form fields and toasts stable and solid. Liquid Glass uses geometry-generated, size-aware RGB refraction and static environment normal/rim specular for navigation, the focused entry or primary entry form, floating surfaces including visible toasts, modals, and the lexical network. Ready Q3 surfaces use a single dynamic optical filter that applies role-aware blur before refraction over a neutral tint; shared UI radii define the visible outline, while smoothly blended overlapping edge normals and exposed-edge models prevent diagonal corner seams without narrowing straight-edge optics. Micro controls keep lightweight CSS volume, while pending, too-small, or unsupported optics use ordinary blur and accessibility modes use solid materials.
...
- 通过可扩展皮肤菜单选择经典、层叠玻璃或液态玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。层叠玻璃将互斥显示的词条查看态与主编辑表单作为同一个聚焦工作区外壳，同时让表单字段和 toast 保持稳定实色。液态玻璃为导航、当前词条或主编辑表单、包含可见 toast 的浮层、modal 与词汇网络生成按实际尺寸适配的 RGB 几何折射，并以静态环境光合成法线/rim 镜面；Q3 就绪表面使用单一动态光学 filter，在折射前按角色模糊背景采样并叠加中性 tint。可见轮廓继续使用共享 UI 圆角，重叠边带通过连续混合法线避免四角斜线，同时不压窄直边折射。微型控件保留轻量 CSS 体积，光学等待、表面过小或能力不可用时使用普通模糊，辅助模式使用实色材质。
...
The lab compares the production geometry/filter engine with two isolated, source-attributed research renderers: the original `archisvaze/liquid-glass` Reference Baseline and an MIT-licensed SDF Baseline adapted from `PallavAg/liquid-glass-web-react`. It does not call application APIs or persist parameter changes.

该实验页可并列比较生产几何/滤镜引擎、隔离的 `archisvaze/liquid-glass` Reference Baseline，以及改编自 MIT 许可 `PallavAg/liquid-glass-web-react` 的 SDF Baseline；它不调用应用 API，也不会持久化参数改动。
