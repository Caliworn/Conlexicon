...
- Controls can opt into one neutral recipe (tone alone, without emphasis), or accent/danger with an explicit outline, tinted or solid emphasis. A shared resolver selects default, hover, pressed and disabled paint; Liquid Glass neutral/outline controls consume the same resolved state with skin-specific tint strength. Corpus layer/unit reorder buttons are the first neutral consumers. B/I/SC, document/corpus mode groups and analysis/quality tabs share selected, selected-hover, selected-pressed and selected-disabled recipes; menus and other unconverted controls retain their appearance. Semantic recipes do not themselves determine Q3 eligibility; tinted/solid optical adaptation remains pending.
