...
- Tabbed data analysis with a four-card, on-demand SQLite overview for lexicon size, data coverage, part-of-speech distribution, and editing activity; topology-backed root-family details stay deferred until opened, while IPA auto-generation checks reuse one cached outcome summary across match variants.
...
- 分标签页数据分析：总览按需异步读取 SQLite 轻量 widget，以四张卡片展示词汇规模、资料覆盖、词性分布和编辑活动；词根、衍生词与多来源词条复用结构筛选，词根家族等拓扑统计延后到详情页打开时计算。IPA 自动生成检查也按需异步加载，并由一份互斥 outcome 摘要提供一致、宽松不一致和严格不一致三类视图。质量检查拥有独立页面，支持按优先度和检查模块查看问题。
