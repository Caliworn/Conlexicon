...
- 通过可扩展皮肤菜单选择经典、层叠玻璃或液态玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。层叠玻璃将互斥显示的词条查看态与主编辑表单作为同一个聚焦工作区外壳，同时让表单字段和 toast 保持稳定实色。液态玻璃为导航、当前词条或主编辑表单、包含可见 toast 的浮层、modal 与词汇网络生成按实际尺寸适配的 RGB 几何折射，并以静态环境光合成法线/rim 镜面；工具导航是一张悬浮在全宽画布上的内嵌连续光学卡片，内部使用平面目的地行、非光学 active 胶囊和分隔线工具组，不再堆叠装饰性玻璃卡片。展开、窄栏、移动顶栏和抽屉形态均暴露并折射四条圆角边。Q3 就绪表面使用单一动态光学 filter，在折射前按角色模糊背景采样并叠加中性 tint。每套皮肤分别拥有定义可见轮廓的角色化 UI 圆角；精确轮廓距离与连续四边光学场共同避免贴图斜向分区，同时不压窄直边折射。微型控件保留轻量 CSS 体积；光学等待、尺寸变化、表面过小或能力不可用时使用普通模糊，辅助模式使用实色材质。
