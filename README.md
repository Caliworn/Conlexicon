...
- Classic, Layered Glass, and Liquid Glass skins selected from an extensible skin menu, dark mode, and Chinese/English UI switching, with the global interface skin, theme, and language remembered in `data/index.json`. Liquid Glass uses geometry-generated, size-aware RGB refraction and static environment normal/rim specular for navigation, the focused entry, floating surfaces, modals, and the lexical network. Ready Q3 surfaces use only the dynamic optical filter over a low-alpha neutral tint; micro controls keep lightweight CSS volume, while pending or unsupported optics use ordinary blur and accessibility modes use solid materials.
...
- 通过可扩展皮肤菜单选择经典、层叠玻璃或液态玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。液态玻璃为导航、当前词条、浮层、modal 与词汇网络生成按实际尺寸适配的 RGB 几何折射，并以静态环境光合成法线/rim 镜面；Q3 就绪表面只在低透明中性 tint 上使用动态光学 filter。微型控件保留轻量 CSS 体积，光学等待或不可用时使用普通模糊，辅助模式使用实色材质。
