...
- Classic/Layered Glass skins selected from an extensible skin menu, dark mode, and Chinese/English UI switching, with the global interface skin, theme, and language remembered in `data/index.json`.
...
- 通过可扩展皮肤菜单选择经典/层叠玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。
