...
- Classic, Layered Glass, and Liquid Glass skins selected from an extensible skin menu, dark mode, and Chinese/English UI switching, with the global interface skin, theme, and language remembered in `data/index.json`. Liquid Glass combines bounded soft/strong SVG refraction with pointer-coalesced caustics on a strict popup allowlist, plus ordinary-blur and solid fallbacks for unsupported or accessibility modes.
...
- 通过可扩展皮肤菜单选择经典、层叠玻璃或液态玻璃皮肤，并支持暗黑模式和中英界面切换；全局界面皮肤、主题和语言会记忆在 `data/index.json` 中。液态玻璃将有界 soft/strong SVG 折射与严格浮层白名单上的合帧指针焦散结合，并为不支持滤镜或辅助模式提供普通模糊和实色降级。
