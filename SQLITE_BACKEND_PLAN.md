...
本节记录当前 `SqliteDictionaryRepository` 实际创建的 schema。它已经不再只是初始草案：词条核心、释义、标签、来源和词条级形态表实例已经关系化；低频或尚未定型的模块继续放在 `module_blobs` 中。开发期 SQLite schema 中间态不承诺兼容迁移，测试库不匹配时从 JSON 重新生成。
...
当前 schema 已明确移除 `entries.entry_json`。SQLite 不再保存完整词条 JSON 副本，而是从 `entries`、`definitions`、`entry_tags`、`entry_sources` 和 `entry_morphology_tables` 组装前端仍需使用的完整 entry object。
...
- 删除词条必须由后端在事务中删除定义、标签、来源、形态表实例等子对象。
...
第一版不要盲目为 IPA、形态学全局配置、文档和语料预建关系表。当前真实边界是：

- IPA 规则：先放 `module_blobs("ipa")`。
- 形态学全局配置：先放 `module_blobs("morphology")`。
- 词条级形态表实例：已拆为 `entry_morphology_tables`，当前用于保存旧 `entry.morphology` 对应的一条实例，并为未来“一词条多形态表”预留 `position`、`title`、`notes` 和 `table_id`。
- 语言文档：先放 `module_blobs("docs")` 或后续单独文本表。
- 语料库：后续进入语料升级时再正式 SQL 化；不要在第一版 SQLite 主存储里预先建一套未使用的 corpus schema。
- 词条级形态 override：当前作为 `entry_morphology_tables.overrides_json` 保存；如果未来 override 内部字段需要搜索、排序或局部 patch，再继续拆细。
...
idx_entry_morphology_tables_entry_id
```
...
当前状态摘要：`lib/sqlite-dictionary-repository.js` 已使用 `node:sqlite` 初始化 `.sqlite` 文件、`schema_migrations`、`dictionary_meta`、`module_blobs`、词条 projection 表和第一批索引；`entries.entry_json` 已移除。`saveEntry()`、`deleteEntry()`、`patchEntries()`、metadata/settings/docs/corpus/morphology/IPA 模块保存都已改为 SQL 级写入。单条读取、列表读取、导出快照、词源关系、facets 和无搜索词根分组已从 SQL 表读取或组装；全文/模糊/动态形态搜索以及带搜索条件的 `queryRootGroups()` 仍复用共享 JS 语义，但输入对象来自 SQL 表而不是 JSON repository。`server.js` 默认使用 SQLite repository；`CONLEXICON_REPOSITORY=json` 可显式切回 JSON repository。
