...
| `readState()` / `listDictionaries()` / `getDictionaryMeta()` | SQLite repository 已只读 `dictionary_meta`，并即时 SQL 计算 `summary.entryCount/rootCount`；这些派生统计不写入 metadata | 否 | 前端已改为轻量 state + active dictionary snapshot；默认 SQLite 路径继续做真实词典 smoke |
...
| `saveCorpusChanges()` / `queryCorpusUnits()` / `getCorpusBlock()` | 保存已直接更新 `module_blobs.corpus` 并返回 `{ id, updatedAt, corpus }`；语料读取仍从 corpus blob 取值；语料保存仍做局部实体 ID 校验 | 语料内容仍是 blob，不是关系表 | 第一版可暂保留；真正大语料阶段再独立 SQL 化 |
...
| 全文 / fuzzy / 动态形态搜索 | 严格及 fuzzy 查询均按字段读取 `entry_search_values` 与 `entry_morphology_search_values`，形态单字段和静态+形态混合查询均返回统一 `searchHits`；fuzzy 通过连接级确定性函数复用共享评分 | 不再依赖完整 snapshot、逐词条动态形态生成或全部候选 records 的 Node.js 物化；fuzzy 仍需线性扫描所选 projection records | 继续以真实基准评估查询形状和候选索引 |
...
| `queryRootGroups()` | 词根与衍生词 ID 拓扑按独立 relation generation 稳定缓存；无搜索和带搜索查询只生成排序/匹配视图，搜索命中直接读取静态与形态 projection | 否；不再导出完整 snapshot 或重复构建关系；无关保存只刷新轻量排序记录 | fuzzy projection 仍是线性扫描，可继续结合真实基准评估候选索引；后续可让词汇网络和质量检查复用同一拓扑 |
...
| 契约测试 | SQLite repository 已跑通当前完整 repository contract | 否 | 主路径语义变化验证 SQLite contract；JSON 仅运行 legacy/debug 与导入转换所需的定向检查 |
...
S3.1 已在共享搜索模型中定义带来源定位的 value record，并固定后续 `/entries` 的 `searchHits` 形状；具体字段见 `API_CONTRACT.md`。S3.2 已把静态字段 records 写入 SQLite，当前表结构为：
...
S3.3 已把静态严格查询切换到该表：按独立 value 执行 `instr(normalized_value, ?)`，避免导出完整 snapshot，并为当前页返回 `searchHits`。该路径覆盖非 ASCII、NFC、Unicode case folding 和自定义等价规则；前端直接消费命中定位生成摘要。任意子串 `instr` 仍不能仅靠普通 B-tree 索引获得完整加速。

### 8.3 S4 形态搜索 projection

S4 第一阶段新增独立的 `entry_morphology_search_values`：
...
严格查询现已按所选字段同时构造两张 projection 的 `EXISTS ... OR ...` 条件。fuzzy 查询先用结构化 SQL 条件收窄词条，再通过连接级 `conlexicon_fuzzy_match()` 复用共享评分。SQL 只返回命中 ID，repository 只为当前页回读完整命中 records；大型候选集顺序扫描 projection，小型结构筛选结果才使用 ID 范围查询。两条路径均不再导出完整 snapshot 或逐词条动态生成形态，并返回与共享 record 契约一致的 `searchHits`。10k 形态压力词典、`limit=10000`、7 轮独立进程基准中，`body` 的全字段/静态字段 fuzzy 约为 251ms/162ms，`bdy` 约为 251ms/159ms。形态 fuzzy-only 查询 `qna` 命中 3186 条时约为 218ms。fuzzy 仍线性扫描 records，后续是否加入真正候选索引应由真实词典基准决定。
