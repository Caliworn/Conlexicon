...
设置可先用 `key/value_json` 保持灵活性；后续对高频查询字段再拆表。

反推 API 契约：

- 词典 metadata 保存应独立于词典内容保存。
- 设置保存应是模块级端点，例如其他设置、IPA、形态学、文档配置、语料配置。
...
第一版可把规则和表格主体保存在 JSON 字段里，重点是拆出独立 ID、顺序和更新时间。动态生成的形态形式不应直接当作永久真相；可在索引表里缓存。
...
corpus_layers(
  id TEXT PRIMARY KEY,
  block_id TEXT NOT NULL REFERENCES corpus_blocks(id) ON DELETE CASCADE,
  position INTEGER NOT NULL,
  label TEXT,
  metadata_json TEXT
)
...
corpus_units(
  id TEXT PRIMARY KEY,
  block_id TEXT REFERENCES corpus_blocks(id) ON DELETE CASCADE,
  layer_id TEXT REFERENCES corpus_layers(id) ON DELETE CASCADE,
  position INTEGER NOT NULL,
  text TEXT,
  gloss_json TEXT,
  metadata_json TEXT,
...
- 语料库不能长期只通过整份 corpus 模块保存。
- 至少需要 block 列表、block 详情、unit 查询、unit 保存、排序变更和删除端点。
- 语料单元的父级不变量必须由后端事务保障：孤立、直属 block、属于 layer 三者只能选一。
...
idx_corpus_units_block_position
idx_corpus_units_layer_position
```
...
2. 实现 JSON 导入到 SQLite、SQLite 导出为 JSON。
