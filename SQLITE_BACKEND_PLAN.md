...
| 全文 / fuzzy / 动态形态搜索 | 严格及 fuzzy 查询均按字段读取 `entry_search_values` 与 `entry_morphology_search_values`，形态单字段和静态+形态混合查询均返回统一 `searchHits`；fuzzy 在规范化 projection 值上复用共享有序子序列 matcher | 不再依赖完整 snapshot 或逐词条动态形态生成；fuzzy 仍需线性扫描所选 projection records | 后续如基准需要，再设计候选索引/FTS/ngram；不能简单用 `LIKE` 替代现有 fuzzy 语义 |
...
严格查询现已按所选字段同时构造两张 projection 的 `EXISTS ... OR ...` 条件。fuzzy 查询先用结构化 SQL 条件收窄词条，再直接遍历所选 projection 的规范化值并复用共享有序子序列 matcher；只为当前页回读完整命中 records。两条路径均不再导出完整 snapshot 或逐词条动态生成形态，并返回与共享 record 契约一致的 `searchHits`。10k 形态压力词典的严格搜索 7 轮中位数由接线前全字段 1830.88ms、仅形态 1832.74ms，降至全字段 257.47ms、仅形态约 95.3ms。fuzzy 投影直读后，查询 `body` 的全字段/静态字段中位数约为 922ms/578ms，查询仅能 fuzzy 命中的 `bdy` 时约为 933ms/585ms；当前复杂度仍与所选 projection records 数量近似线性，后续是否引入候选索引/FTS/ngram 应由继续基准决定。

## 9. 默认切换后的第一批优化建议

以下不阻断默认切换，但适合作为 SQLite 成为默认后的第一批优化：

1. 根据真实词典基准，为严格子串和 fuzzy projection 扫描评估 FTS / ngram / 其他候选索引，减少逐 record 线性匹配。
2. 将带搜索条件的 `queryRootGroups()` 从共享 JS 回退推进到 SQL/FTS/搜索索引。
3. 把数据分析和质量检查推进为按需 API + query planner，而不是前端基于完整 snapshot 重算。
4. 语料库进入独立升级阶段后，再把 `module_blobs.corpus` 拆成正式 SQL 表。
