...
`module_blobs` 是 SQLite 主库里的 JSON 扩展位，不是把设置 SQL 化。第一版可用它保存低频或结构复杂的模块，例如 `settings`、`ipa`、`docs`、暂未拆表的语料配置，以及形态学里尚未进入 DSL v2 的旧 `leftV/rightV` 函数对象设置。只有当某个模块需要独立查询、筛选、排序、索引或局部保存时，才再拆出关系表。
...
- `entry_morphology_groups.title` 保留，因为一个词条未来可手动配置多个形态模板组，词条级显示标题需要独立于模板组名称。
...
| `saveMorphology()` | 已保留形态语法校验；旧 `leftV/rightV` 函数对象设置继续写入 `module_blobs.morphology`，形态模板组/子表/单元格写入 SQL projection；形态保存仍做局部实体 ID 校验 | 返回值仍组装完整 snapshot | 后续 DSL v2 时移除旧函数对象设置，并按新前端模型收窄响应 |
