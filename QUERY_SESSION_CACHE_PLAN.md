...
该问题暂不在 Q3 后立即局部修补，按以下顺序处理：

1. Q4 数据窗口化已经完成，列表状态、cursor、窗口拼接和滚动锚点已成为后续局部渲染边界。
2. 再实现两段式 stale-while-revalidate：请求开始后的 200ms 内保持旧内容原样；超过 200ms 后，词条详情显示遮罩，词条列表保留旧列表但轻微变淡，并使用同一套覆盖式视觉语言显示明确的“正在更新”。不增加第三档提示。
3. 最后把词条切换从全局 `render()` 拆成局部提交：只更新待选/选中卡片、详情和必要的滚动状态，不重建未变化的列表窗口。
4. 详情中的来源解析和衍生词改用共享关系 API/缓存；静态详情先显示，关系区和复杂形态区可按需渐进补齐。完成后，未缓存词条切换的主要等待应只剩单条本地 API 请求，耗时不再随词典总规模显著增长。

请求失败仍直接进入现有错误状态并报告错误，不重试、不回滚，也不把旧内容冒充为成功结果。Q4 已稳定列表所有权和窗口状态，后续可在其上实现该过渡视觉及局部提交。
