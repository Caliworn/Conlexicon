...
2. 两段式 stale-while-revalidate 已实装：查询首窗或词条详情请求开始后的 200ms 内保持旧内容原样；超过 200ms 后，词条详情显示遮罩，词条列表保留旧列表但轻微变淡，并使用同一套覆盖式视觉语言显示明确的“正在更新”。首次加载没有旧内容时仍直接显示加载状态；滚动补载后续窗口只使用窗口局部占位，不遮罩整个列表。
