...
const { QuerySessionCache } = require("./query-session-cache");
...
function normalizeEntryQuery(query = {}) {
  ...
  return {
    ...
    part: normalizeStructuralKey(query.part),
    tags: splitQueryList(query.tags).map(normalizeStructuralKey).filter(Boolean),
...
function canUseSqlSearchProjectionQuery(query) {
  return Boolean(
    query.q
    && !query.fuzzyFields.size
  );
}
...
class SqliteDictionaryRepository {
  ...
  async importDictionarySnapshot(dictionary, { overwrite = true } = {}) {
    ...
    this.invalidateQuerySessions(normalized.id);
  ...
  querySessionCacheStats() {
    return this.querySessionCache.stats();
  }
  ...
  entryQueryWhereClauses(db, dictionary, query) {
    ...
    if (query.part) {
      if (query.part === NO_PART_FILTER_VALUE) {
        ...
        if (!configuredParts.has(query.part)) {
          clauses.push("0 = 1");
        } else {
          clauses.push(`EXISTS (
            SELECT 1 FROM entry_tags part_tags
            WHERE part_tags.entry_id = e.id AND part_tags.tag = ?
          )`);
    ...
    if (query.tags.length) {
      const tags = query.tags.map(normalizeStructuralKey).filter(Boolean);
      if (query.tagMode === "all") {
        tags.forEach((tag) => {
          clauses.push(`EXISTS (
            SELECT 1 FROM entry_tags filter_tags
            WHERE filter_tags.entry_id = e.id AND filter_tags.tag = ?
          )`);
          params.push(tag);
        });
      } else if (tags.length) {
        clauses.push(`EXISTS (
          ...
          WHERE filter_tags.entry_id = e.id
            AND filter_tags.tag IN (${sqlPlaceholders(tags)})
        )`);
        params.push(...tags);
      }
    }
  ...
  entrySearchProjectionWhereClause(query) {
    const clauses = [];
    const params = [];
    const staticFields = [...query.searchFields].filter((field) => STATIC_ENTRY_SEARCH_FIELDS.has(field));
    if (staticFields.length) {
      clauses.push(`EXISTS (
        SELECT 1 FROM entry_search_values search_value
        WHERE search_value.entry_id = e.id
          AND search_value.field IN (${sqlPlaceholders(staticFields)})
          AND instr(search_value.normalized_value, ?) > 0
      )`);
      params.push(...staticFields, query.q);
    }
    if (query.searchFields.has("morphology")) {
      clauses.push(`EXISTS (
        SELECT 1 FROM entry_morphology_search_values morphology_search_value
        WHERE morphology_search_value.entry_id = e.id
          AND instr(morphology_search_value.normalized_value, ?) > 0
      )`);
      params.push(query.q);
    }
    return {
  ...
  entrySummariesFromRows(db, rows = [], dictionary = {}) {
    ...
    collectRows((placeholders) => `
      SELECT entry_id AS entryId, position, meaning
      FROM definitions
      WHERE entry_id IN (${placeholders})
      ORDER BY entry_id ASC, position ASC
    `, (row) => {
      if (!definitionsByEntry.has(row.entryId) && row.meaning) {
        definitionsByEntry.set(row.entryId, row.meaning);
      }
    });
    collectRows((placeholders) => `
    ...
    return rows.map((row) => entrySummary({
      ...
      definitions: definitionsByEntry.has(row.id)
        ? [{ meaning: definitionsByEntry.get(row.id) }]
        : [],
    }, dictionary));
  }
  ...
  queryEntriesSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    ...
    const page = query.include === "full"
      ? this.entriesFromRows(db, rows)
      : this.entrySummariesFromRows(db, rows, dictionary);
  ...
  entrySearchHitsFromDatabase(db, entryIds, query) {
    ...
    const addHit = (entryId, hit) => {
      if (!hitsByEntry.has(entryId)) {
        hitsByEntry.set(entryId, []);
      }
      hitsByEntry.get(entryId).push(hit);
    };
    const fields = [...query.searchFields].filter((field) => STATIC_ENTRY_SEARCH_FIELDS.has(field));
    chunkArray(entryIds).forEach((entryIdChunk) => {
      if (fields.length) {
        db.prepare(`
          SELECT entry_id AS entryId, field, source_type AS sourceType, source_id AS sourceId,
            source_position AS sourcePosition, value_type AS valueType, raw_value AS value
        ...
        `).all(...entryIdChunk, ...fields, query.q).forEach((row) => {
          addHit(row.entryId, {
            field: row.field,
            value: row.value || "",
      ...
      if (query.searchFields.has("morphology")) {
        db.prepare(`
          ...
          WHERE entry_id IN (${sqlPlaceholders(entryIdChunk)})
            AND instr(normalized_value, ?) > 0
          ORDER BY entry_id ASC, position ASC
        `).all(...entryIdChunk, query.q).forEach((row) => {
          addHit(row.entryId, {
            field: "morphology",
            value: row.value || "",
  ...
  entryFuzzyMatchedIdsFromDatabase(db, entryIds, query) {
    const matchedIds = new Set();
    const eligibleIds = new Set(entryIds);
    const staticFields = [...query.searchFields].filter((field) => STATIC_ENTRY_SEARCH_FIELDS.has(field));
    const fuzzyStaticFields = staticFields.filter((field) => query.fuzzyFields.has(field));
    const strictStaticFields = staticFields.filter((field) => !query.fuzzyFields.has(field));
    const entryIdScopes = entryIds.length > 1000 ? [null] : chunkArray(entryIds, 500);
    entryIdScopes.forEach((entryIdChunk) => {
      const entryIdClause = entryIdChunk
        ? `entry_id IN (${sqlPlaceholders(entryIdChunk)}) AND`
        : "";
      const staticClauses = [];
      const staticParams = entryIdChunk ? [...entryIdChunk] : [];
      if (fuzzyStaticFields.length) {
        staticClauses.push(`(
          field IN (${sqlPlaceholders(fuzzyStaticFields)})
          AND ${SQLITE_FUZZY_MATCH_FUNCTION}(normalized_value, ?) = 1
        )`);
        staticParams.push(...fuzzyStaticFields, query.q);
      }
      if (strictStaticFields.length) {
        staticClauses.push(`(
          field IN (${sqlPlaceholders(strictStaticFields)})
          AND instr(normalized_value, ?) > 0
        )`);
        staticParams.push(...strictStaticFields, query.q);
      }
      if (staticClauses.length) {
        db.prepare(`
          SELECT DISTINCT entry_id AS entryId
          FROM entry_search_values
          WHERE ${entryIdClause} (${staticClauses.join(" OR ")})
        `).all(...staticParams).forEach((record) => {
          if (eligibleIds.has(record.entryId)) {
            matchedIds.add(record.entryId);
          }
        });
      }
      if (query.searchFields.has("morphology")) {
        const morphologyFuzzy = query.fuzzyFields.has("morphology");
        const morphologyMatch = morphologyFuzzy
          ? `${SQLITE_FUZZY_MATCH_FUNCTION}(normalized_value, ?) = 1`
          : "instr(normalized_value, ?) > 0";
        ...
        db.prepare(`
          SELECT DISTINCT entry_id AS entryId
          FROM entry_morphology_search_values
          WHERE ${entryIdClause} ${morphologyMatch}
        `).all(...morphologyParams).forEach((record) => {
          if (eligibleIds.has(record.entryId)) {
            matchedIds.add(record.entryId);
          }
        });
  ...
  queryEntriesSearchProjectionSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    const structural = this.entryQueryWhereClauses(db, dictionary, query);
    const text = this.entrySearchProjectionWhereClause(query);
    const clauses = [...structural.clauses, text.clause];
    const params = [...structural.params, ...text.params];
    ...
    const page = query.include === "full"
      ? this.entriesFromRows(db, rows)
      : this.entrySummariesFromRows(db, rows, dictionary);
    const hitsByEntry = this.entrySearchHitsFromDatabase(db, page.map((entry) => entry.id), query);
  ...
  buildFuzzyEntryQuerySession(id, query, dictionary) {
    const db = this.openDictionaryDatabase(id);
    const structural = this.entryQueryWhereClauses(db, dictionary, query);
    ...
    const eligibleRows = db.prepare(`
      ...
      ${whereSql}
      ORDER BY ${sqlOrderClause(query.sort, "e")}
    `).all(...structural.params);
    const matchedIds = this.entryFuzzyMatchedIdsFromDatabase(
      db,
      eligibleRows.map((row) => row.id),
      query,
    );
    return {
      orderedIds: eligibleRows.filter((row) => matchedIds.has(row.id)).map((row) => row.id),
    };
  }

  async queryEntriesFuzzyProjectionSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    const session = await this.currentQuerySession({
      kind: "entries",
      dictionaryId: id,
      dictionaryUpdatedAt: dictionary.updatedAt,
      query,
      build: () => this.buildFuzzyEntryQuerySession(id, query, dictionary),
    });
    const pageIds = session.orderedIds.slice(query.offset, query.offset + query.limit);
    const pageRows = this.entryRowsByIds(db, pageIds);
    const page = query.include === "full"
      ? this.entriesFromRows(db, pageRows)
      : this.entrySummariesFromRows(db, pageRows, dictionary);
    const hitsByEntry = this.entryFuzzySearchHitsFromDatabase(db, pageRows.map((row) => row.id), query);
    const items = page.map((entry) => ({
      ...entry,
      searchHits: hitsByEntry.get(entry.id) || [],
    }));
  ...
  async queryEntries(id, query = null) {
    ...
    normalizedQuery.normalizeSearchText = searchRuntime.normalizeText;
    normalizedQuery.searchNormalization = searchRuntime.normalization;
    ...
    if (canUseSqlSearchProjectionQuery(normalizedQuery)) {
      return this.queryEntriesSearchProjectionSql(id, normalizedQuery, dictionary);
    }
    return this.queryEntriesFuzzyProjectionSql(id, normalizedQuery, dictionary);
  }
  ...
  rootGroupSessionRecords(groups = []) {
    return groups.map((group) => ({
      rootId: group.root?.id || "",
      derivedIds: (group.derived || []).map((entry) => entry.id).filter(Boolean),
  ...
  serializeRootGroupSessionPage(id, query, dictionary, session) {
    const db = this.openDictionaryDatabase(id);
    const page = session.groups.slice(query.offset, query.offset + query.limit);
    const entryIds = [...new Set(page.flatMap((group) => [group.rootId, ...group.derivedIds]).filter(Boolean))];
    const rows = this.entryRowsByIds(db, entryIds);
    const entries = query.include === "full"
      ? this.entriesFromRows(db, rows)
      : this.entrySummariesFromRows(db, rows, dictionary);
    ...
    return {
      items: page.map((group) => ({
        ...
        derivedEntries: group.derivedIds.map((entryId) => entriesById.get(entryId)).filter(Boolean),
  ...
  async queryRootGroupsSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    const session = await this.currentQuerySession({
      kind: "rootGroups",
      dictionaryId: id,
      ...
      query,
      build: () => ({
        groups: this.rootGroupSessionRecords(this.rootGroupsFromDatabase(db, { sort: query.sort })),
      }),
    });
    return this.serializeRootGroupSessionPage(id, query, dictionary, session);
  }
  ...
  async queryRootGroups(id, query = {}) {
    ...
    if (canUseSqlRootGroupsQuery(normalizedQuery)) {
      return this.queryRootGroupsSql(id, normalizedQuery, dictionaryContext);
    }
    ...
    const session = await this.currentQuerySession({
      kind: "rootGroups",
      dictionaryId: id,
      dictionaryUpdatedAt: dictionaryContext.updatedAt,
      ...
      build: () => {
        ...
        const groups = rootModeGroups(dictionary, {
          ...
          normalizeText,
          compareEntries,
          matchesEntry: (entry) => entryMatchesSearchText(entry, dictionary, normalizedQuery.rawQ, {
            fields: normalizedQuery.searchFields,
            fuzzyFields: normalizedQuery.fuzzyFields,
            normalizeText: searchRuntime.normalizeText,
          }),
        });
        return { groups: this.rootGroupSessionRecords(groups) };
      },
    });
