...
class SqliteDictionaryRepository {
  ...
  dictionaryMetadataFromDatabase(id) {
    ...
    return {
      ...
      summary: this.dictionarySummaryFromDatabase(db, id),
    };
  }
  ...
  prepareQueryCursor(kind, dictionaryId, dictionaryUpdatedAt, query) {
    ...
    query.offset = query.windowOffset === null ? offset : query.windowOffset;
    return query;
  }
  ...
  dictionarySummaryFromDatabase(db, dictionaryId) {
    return {
      entryCount: Number(db.prepare("SELECT COUNT(*) AS count FROM entries").get()?.count || 0),
      rootCount: this.currentRootTopology(dictionaryId, db).groups.length,
    };
  }
  ...
  queryEntriesSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    const { clauses, params } = this.entrySqlQueryWhere(db, dictionary, query);
  ...
  buildFuzzyEntryQuerySession(id, query, dictionary) {
    ...
    const orderedIds = eligibleRows.filter((row) => matchedIds.has(row.id)).map((row) => row.id);
    return {
      orderedIds,
  ...
  entryFuzzyQuerySession(id, query, dictionary) {
  ...
  async queryEntriesFuzzyProjectionSql(id, query, dictionary = this.dictionaryQueryContext(id)) {
    const db = this.openDictionaryDatabase(id);
    const session = await this.entryFuzzyQuerySession(id, query, dictionary);
  ...
  entrySqlQueryWhere(db, dictionary, query) {
    const structural = this.entryQueryWhereClauses(db, dictionary, query);
    if (!query.q) {
      return structural;
    }
    ...
    return {
      clauses: [...structural.clauses, text.clause],
      params: [...structural.params, ...text.params],
    };
  }
  ...
  async locateEntryQueryWindow(id, entryId, query = {}) {
    ...
    const normalizedQuery = normalizeEntryQuery({
      ...query,
      cursor: "",
      windowOffset: "",
    });
    ...
    let total = 0;
    let pageResult = null;
    if (canUseSqlEntryQuery(normalizedQuery) || canUseSqlSearchProjectionQuery(normalizedQuery)) {
      const location = this.entrySqlQueryLocation(db, dictionary, normalizedQuery, entryId);
      resultIndex = location.resultIndex;
      total = location.total;
      if (resultIndex >= 0) {
        normalizedQuery.offset = Math.floor(resultIndex / normalizedQuery.limit) * normalizedQuery.limit;
        pageResult = canUseSqlEntryQuery(normalizedQuery)
          ? this.queryEntriesSql(id, normalizedQuery, dictionary)
          : this.queryEntriesSearchProjectionSql(id, normalizedQuery, dictionary);
      }
    } else {
      const session = await this.entryFuzzyQuerySession(id, normalizedQuery, dictionary);
      resultIndex = session.entryIndexById.get(entryId) ?? -1;
      total = session.orderedIds.length;
      if (resultIndex >= 0) {
        normalizedQuery.offset = Math.floor(resultIndex / normalizedQuery.limit) * normalizedQuery.limit;
        pageResult = await this.queryEntriesFuzzyProjectionSql(id, normalizedQuery, dictionary);
      }
    }
    ...
    return {
      location: {
        ...
        entryId,
        resultIndex,
        windowIndex: Math.floor(resultIndex / normalizedQuery.limit),
  ...
  rootGroupSessionRecordsFromTopology(db, topology, query) {
    ...
    const matchedIds = this.entryFuzzyMatchedIdsFromDatabase(db, topology.entryIds, query);
    return groups.map((group) => {
