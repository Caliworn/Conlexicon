...
class SqliteDictionaryRepository {
  ...
  entryPartStatsFromDatabase(db, dictionary) {
    ...
    if (configuredParts.length) {
      partRows = db.prepare(`
        SELECT tag, COUNT(*) AS entryCount
        FROM entry_tags
      ...
      `).get(...configuredParts)?.entryCount || 0);
    } else {
    ...
    partRows.forEach((row) => {
      partMap.set(row.tag, Number(row.entryCount || 0));
    });
    return {
      parts: [...partMap.entries()]
        .map(([part, entryCount]) => ({ part, displayLabel: displayTag(part, dictionary), entryCount })),
      noPartOfSpeechCount,
    };
  }
  ...
  async getEntryFacets(id) {
    ...
    db.prepare(`
      ...
      GROUP BY tag
    `).all().forEach((row) => {
      tagMap.set(row.tag, Number(row.entryCount || 0));
    });
