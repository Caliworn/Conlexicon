...
function canUseSqlStaticTextEntryQuery(query) {
  // SQLite's built-in lower() only guarantees ASCII case folding. Restrict this
  // direct path accordingly so non-ASCII/IPA searches keep the shared JS
  // normalizer's locale-aware semantics instead of silently changing results.
...
class SqliteDictionaryRepository {
  ...
  entryStaticTextWhereClause(dictionary, query) {
    ...
    const addTextClause = (expression) => {
      clauses.push(`instr(lower(COALESCE(${expression}, '')), ?) > 0`);
      params.push(query.q);
    };

    if (query.searchFields.has("lemma")) {
      clauses.push("instr(e.sort_key, ?) > 0");
      params.push(query.q);
    }
    ...
    if (query.searchFields.has("tags")) {
      const tagClauses = [`EXISTS (
        ...
        WHERE raw_tags.entry_id = e.id
          AND instr(lower(raw_tags.tag), ?) > 0
      )`];
      params.push(query.q);
      const displayMatchedTags = Object.entries(dictionary.settings?.tagDisplayMap || {})
        .filter(([, display]) => normalizeText(display).includes(query.q))
    ...
    if (query.searchFields.has("definitions")) {
      clauses.push(`EXISTS (
        ...
        WHERE definition_search.entry_id = e.id
          AND instr(lower(definition_search.meaning), ?) > 0
      )`);
      params.push(query.q);
    }
