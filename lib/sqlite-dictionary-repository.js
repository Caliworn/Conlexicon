...
class SqliteDictionaryRepository {
  ...
  entryFuzzyMatchedIdsFromDatabase(db, entryIds, query) {
    ...
    chunkArray(entryIds, 5000).forEach((entryIdChunk) => {
      if (fields.length) {
        db.prepare(`
          SELECT entry_id AS entryId, field, normalized_value AS normalizedValue
          FROM entry_search_values
          WHERE entry_id IN (${sqlPlaceholders(entryIdChunk)})
            AND field IN (${sqlPlaceholders(fields)})
        `).all(...entryIdChunk, ...fields).forEach((record) => {
          if (!matchedIds.has(record.entryId) && normalizedTextMatches(
      ...
      if (query.searchFields.has("morphology")) {
        db.prepare(`
          SELECT entry_id AS entryId, normalized_value AS normalizedValue
          FROM entry_morphology_search_values
          WHERE entry_id IN (${sqlPlaceholders(entryIdChunk)})
        `).all(...entryIdChunk).forEach((record) => {
          if (!matchedIds.has(record.entryId) && normalizedTextMatches(
            record.normalizedValue || "",
            query.q,
            query.fuzzyFields.has("morphology"),
          )) {
            matchedIds.add(record.entryId);
          }
        });
