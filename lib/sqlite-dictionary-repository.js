...
class SqliteDictionaryRepository {
  constructor({
    ...
    this.normalizeUiSkin = normalizeUiSkin || ((value) => ["layered-glass", "liquid-glass"].includes(value) ? value : "classic");
  ...
  entriesFromRows(db, rows = []) {
    ...
    collectRows((placeholders) => `
      SELECT entry_id AS entryId, target_entry_id AS targetEntryId, source_text AS sourceText
      FROM entry_sources
      WHERE entry_id IN (${placeholders})
      ORDER BY entry_id ASC, target_entry_id IS NULL, position ASC
    `, (row) => {
      if (!sourcesByEntry.has(row.entryId)) {
        sourcesByEntry.set(row.entryId, []);
      }
      sourcesByEntry.get(row.entryId).push({ entryId: row.targetEntryId || "", text: row.sourceText || "" });
    });
  ...
  directDerivedEntries(db, sourceEntry, { sort = "lemmaAsc" } = {}) {
    ...
    return db.prepare(`
      ...
      WHERE s.target_entry_id = ?
        AND e.id <> ?
      ORDER BY ${sqlOrderClause(sort, "e")}
    `).all(sourceEntry.id, sourceEntry.id)
  ...
  entrySourcesFromDatabase(db, entryId) {
    return db.prepare(`
      SELECT source_text AS sourceText, target_entry_id AS targetEntryId
      FROM entry_sources
      WHERE entry_id = ?
      ORDER BY target_entry_id IS NULL, position ASC
    `).all(entryId);
  }
  ...
  rootTopologyFromDatabase(db) {
    ...
    db.prepare(`
      SELECT entry_id AS entryId, target_entry_id AS targetEntryId
      FROM entry_sources
      ORDER BY entry_id ASC, position ASC
    `).all().forEach((row) => {
      ...
      sourceKeysByEntry.get(row.entryId).push(row.targetEntryId);
    });
    ...
    sourceKeysByEntry.forEach((sourceKeys, entryId) => {
      ...
      sourceKeys.forEach((sourceKey) => {
        const source = entriesById.get(sourceKey) || null;
  ...
  async getEntryRelations(id, entryId) {
    ...
    const sources = this.entrySourcesFromDatabase(db, entryId).map((source) => {
      const matchedEntry = source.targetEntryId ? this.getEntryFromDatabase(db, source.targetEntryId) : null;
      return {
        sourceText: matchedEntry?.lemma || source.sourceText,
  ...
  async saveEntry(id, entry, options = {}) {
    ...
    const existingSourceKeys = existingRow
      ? this.entrySourcesFromDatabase(db, nextEntry.id).map((source) => source.targetEntryId || "")
      : [];
    const nextSourceKeys = (nextEntry.etymology?.sources || []).map((source) => source.entryId);
    ...
    try {
      ...
      if (options.sourceBinding) {
        ...
        if (!options.createOnly || typeof binding.ownerEntryId !== "string"
          || !Number.isInteger(binding.sourcePosition) || binding.sourcePosition < 0
          || !Array.isArray(binding.expectedSources) || typeof binding.expectedUpdatedAt !== "string"
          || binding.expectedSources.some((item) => !item || typeof item.entryId !== "string" || typeof item.text !== "string")) {
          throw apiError("Invalid source binding", 400, "invalid_source_binding");
        }
        const owner = this.getEntryFromDatabase(db, binding.ownerEntryId);
        const sources = owner?.etymology?.sources || [];
        const source = sources[binding.sourcePosition];
        if (!owner || owner.id === nextEntry.id || owner.updatedAt !== binding.expectedUpdatedAt
          || sources.length !== binding.expectedSources.length
          || sources.some((item, index) => item.entryId !== binding.expectedSources[index].entryId || item.text !== binding.expectedSources[index].text)
          || !source || source.entryId || !source.text) {
          throw apiError("Source changed before binding", 409, "source_binding_conflict");
        }
        const remaining = sources.filter((_, index) => index !== binding.sourcePosition);
        const updatedOwner = this.normalizeEntryForSave(baseDictionary, {
          ...owner,
          updatedAt: now,
          etymology: { ...owner.etymology, sources: [
            ...remaining.filter((item) => item.entryId),
            { entryId: nextEntry.id, text: nextEntry.lemma || nextEntry.id },
            ...remaining.filter((item) => !item.entryId),
          ] },
        });
        const ownerPosition = db.prepare("SELECT position FROM entries WHERE id = ?").get(owner.id).position;
        this.writeEntryProjection(db, updatedOwner, ownerPosition, baseDictionary);
      }
      this.touchDictionary(db, id, now);
      db.exec("COMMIT");
    } catch (error) {
    ...
    return {
      ...
      ...(options.sourceBinding ? { ownerEntry: this.getEntryFromDatabase(db, options.sourceBinding.ownerEntryId) } : {}),
      summary: this.dictionarySummaryFromDatabase(db),
    };
  }
  ...
  async deleteEntry(id, entryId) {
    ...
    const now = new Date().toISOString();
    const referencingIds = db.prepare("SELECT DISTINCT entry_id AS id FROM entry_sources WHERE target_entry_id = ? AND entry_id <> ?").all(entryId, entryId);
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const context = this.baseDictionarySnapshot(id);
      referencingIds.forEach(({ id: ownerId }) => this.writeEntrySearchProjection(db, this.getEntryFromDatabase(db, ownerId), context));
      this.touchDictionary(db, id, now);
      db.exec("COMMIT");
    } catch (error) {
      db.exec("ROLLBACK");
  ...
  async updatePreferences(preferences) {
    ...
    const nextIndex = {
      ...
      ...(Object.hasOwn(preferences, "uiSkin") ? { uiSkin: preferences.uiSkin } : {}),
    };
    await this.writeIndex(nextIndex);
    return { uiLanguage: nextIndex.uiLanguage, uiTheme: nextIndex.uiTheme, uiSkin: nextIndex.uiSkin };
  }
  ...
  staticSearchEntriesFromDatabase(db) {
    ...
    db.prepare(`
      SELECT entry_id AS entryId, position, target_entry_id AS targetEntryId, source_text AS sourceText
      FROM entry_sources
      ORDER BY entry_id ASC, target_entry_id IS NULL, position ASC
    `).all().forEach((row) => {
      byId.get(row.entryId)?.etymology.sources.push({ entryId: row.targetEntryId || "", text: row.sourceText || "" });
    });
    db.prepare(`
  ...
  writeEntryProjection(db, entry, position, dictionary) {
    const previousLemma = db.prepare("SELECT lemma FROM entries WHERE id = ?").get(entry.id)?.lemma;
    entry.etymology.sources = entry.etymology.sources.map((source) => {
      if (!source.entryId) return source;
      const target = source.entryId === entry.id ? entry : db.prepare("SELECT id, lemma FROM entries WHERE id = ?").get(source.entryId);
      if (!target) throw apiError("Source target not found", 400, "invalid_source_target", { entryId: source.entryId });
      return { entryId: source.entryId, text: target.lemma || target.id };
    });
    ...
    const insertSource = db.prepare(`
      INSERT INTO entry_sources(entry_id, position, source_text, target_entry_id)
      VALUES (?, ?, ?, ?)
    `);
    (entry.etymology?.sources || []).forEach((source, sourceIndex) => {
      insertSource.run(entry.id, sourceIndex, source.text, source.entryId || null);
    });
    ...
    if (previousLemma !== entry.lemma) {
      const incoming = db.prepare("SELECT DISTINCT entry_id AS id FROM entry_sources WHERE target_entry_id = ? AND entry_id <> ?").all(entry.id, entry.id);
      db.prepare("UPDATE entry_sources SET source_text = ? WHERE target_entry_id = ?").run(entry.lemma || entry.id, entry.id);
      incoming.forEach(({ id }) => this.writeEntrySearchProjection(db, this.getEntryFromDatabase(db, id), dictionary));
    }
  ...
  writeDictionaryToDatabase(db, dictionary) {
    const targets = new Map((dictionary.entries || []).map((entry) => [entry.id, entry]));
    (dictionary.entries || []).forEach((entry) => {
      entry.etymology.sources = entry.etymology.sources.map((source) => {
        if (!source.entryId) return source;
        const target = targets.get(source.entryId);
        if (!target) throw apiError("Source target not found", 400, "invalid_source_target", { entryId: source.entryId });
        return { entryId: target.id, text: target.lemma || target.id };
      });
    });
    const now = new Date().toISOString();
    db.exec("BEGIN IMMEDIATE");
  ...
  async readIndex() {
    ...
    return {
      ...
      uiSkin: this.normalizeUiSkin(index.uiSkin),
    };
  }
  ...
  async writeIndex(index) {
    ...
    await this.writeJson(this.indexPath, {
      ...
      uiSkin: this.normalizeUiSkin(index.uiSkin ?? existing.uiSkin),
    });
  }
...
CREATE INDEX IF NOT EXISTS idx_entry_sources_target_entry_id ON entry_sources(target_entry_id);
