...
function entryMatchesQuery(entry, dictionary, query) {
  const normalizeEntrySearchText = query.normalizeSearchText || normalizeText;
  if (query.q && !entryMatchesSearchText(entry, dictionary, query.rawQ ?? query.q, {
    fields: query.searchFields,
    fuzzyFields: query.fuzzyFields,
    normalizeText: normalizeEntrySearchText,
  })) {
    return false;
  }
  const parts = entryParts(entry, dictionary);
  if (query.part) {
    ...
    } else if (!parts.some((part) => normalizeStructuralKey(part) === query.part)) {
      return false;
    }
  }
  if (query.tags.length) {
    const entryTags = new Set((entry.tags || []).map(normalizeStructuralKey));
    const matches = query.tags.map((tag) => entryTags.has(normalizeStructuralKey(tag)));
...
function canUseSqlEntryQuery(query) {
  return !query.q && !query.fuzzyFields.size;
}

function canUseSqlStaticTextEntryQuery(query) {
  // S2 can safely keep the direct SQL path only for strict, unmodified ASCII
  // text. NFC does not alter ASCII substrings; case folding and custom rules
  // require the shared JS matcher until S3 adds normalized SQL projections.
  // The shared matcher evaluates each independent field value separately, so
  // printable ASCII phrases retain the same semantics in SQL EXISTS clauses.
  return Boolean(
    query.q
    && /^[\x20-\x7E]+$/.test(query.q)
    && !query.searchFields.has("morphology")
    && !query.fuzzyFields.size
    && !query.searchNormalization?.caseFolding
    && !query.searchNormalization?.customRules?.length
  );
}
...
function entryEntityIdRecords(entry = {}) {
  ...
  (entry.morphologyGroups || []).forEach((group) => {
    if (group.id) {
      records.push({ id: group.id, type: "entry morphology group", scope: "entry", ownerId: entry.id });
    }
  });
  return records;
}
...
class SqliteDictionaryRepository {
  ...
  entryStaticTextWhereClause(dictionary, query) {
    const clauses = [];
    const params = [];
    const addTextClause = (expression) => {
      clauses.push(`instr(COALESCE(${expression}, ''), ?) > 0`);
      params.push(query.q);
    };

    if (query.searchFields.has("lemma")) {
      addTextClause("e.lemma");
    }
    if (query.searchFields.has("pronunciation")) {
      addTextClause("e.pronunciation");
    }
    if (query.searchFields.has("tags")) {
      const tagClauses = [`EXISTS (
        SELECT 1 FROM entry_tags raw_tags
        WHERE raw_tags.entry_id = e.id
          AND instr(raw_tags.tag, ?) > 0
      )`];
      params.push(query.q);
      const displayMatchedTags = Object.entries(dictionary.settings?.tagDisplayMap || {})
        .filter(([, display]) => query.normalizeSearchText(display).includes(query.q))
        .map(([tag]) => normalizeStructuralKey(tag))
        .filter(Boolean);
      if (displayMatchedTags.length) {
        tagClauses.push(`EXISTS (
          SELECT 1 FROM entry_tags display_tags
          WHERE display_tags.entry_id = e.id
            AND display_tags.tag IN (${sqlPlaceholders(displayMatchedTags)})
        )`);
        params.push(...displayMatchedTags);
      }
      clauses.push(`(${tagClauses.join(" OR ")})`);
    }
    if (query.searchFields.has("definitions")) {
      clauses.push(`EXISTS (
        SELECT 1 FROM definitions definition_search
        WHERE definition_search.entry_id = e.id
          AND instr(definition_search.meaning, ?) > 0
      )`);
      params.push(query.q);
    }
    if (query.searchFields.has("examples")) {
      clauses.push(`EXISTS (
        SELECT 1 FROM definitions example_search
        WHERE example_search.entry_id = e.id
          AND instr(example_search.example, ?) > 0
      )`);
      params.push(query.q);
    }
    if (query.searchFields.has("notes")) {
      const noteClauses = ["instr(e.notes, ?) > 0"];
      params.push(query.q);
      noteClauses.push(`EXISTS (
        SELECT 1 FROM definitions definition_note_search
        WHERE definition_note_search.entry_id = e.id
          AND instr(definition_note_search.notes, ?) > 0
      )`);
      params.push(query.q);
      clauses.push(`(${noteClauses.join(" OR ")})`);
    }
    if (query.searchFields.has("etymology")) {
      const etymologyClauses = ["instr(e.etymology_description, ?) > 0"];
      params.push(query.q);
      etymologyClauses.push(`EXISTS (
        SELECT 1 FROM entry_sources source_search
        WHERE source_search.entry_id = e.id
          AND instr(source_search.source_text, ?) > 0
      )`);
      params.push(query.q);
      clauses.push(`(${etymologyClauses.join(" OR ")})`);
    }
  ...
  entriesFromRows(db, rows = []) {
    ...
    collectRows((placeholders) => `
      SELECT id, entry_id AS entryId, position, template_group_id AS templateGroupId, title, notes, created_at AS createdAt, updated_at AS updatedAt
    ...
    `, (row) => {
      ...
      morphologyGroupIds.push(row.id);
    });

    if (morphologyGroupIds.length) {
      chunkArray(morphologyGroupIds).forEach((chunk) => {
        db.prepare(`
          SELECT entry_morphology_group_id AS groupId, template_table_id AS tableId, row_index AS rowIndex, column_index AS columnIndex, value
          FROM entry_morphology_cell_overrides
          WHERE entry_morphology_group_id IN (${sqlPlaceholders(chunk)})
          ORDER BY entry_morphology_group_id ASC, template_table_id ASC, row_index ASC, column_index ASC
        `).all(...chunk).forEach((row) => {
          for (const groups of morphologyByEntry.values()) {
  ...
  queryEntriesStaticTextSql(id, query) {
    const db = this.openDictionaryDatabase(id);
    const dictionary = this.baseDictionarySnapshot(id);
    const structural = this.entryQueryWhereClauses(db, dictionary, query);
    const text = this.entryStaticTextWhereClause(dictionary, query);
    const clauses = [...structural.clauses, `(${text.clauses.join(" OR ")})`];
  ...
  async queryEntries(id, query = null) {
    ...
    const baseDictionary = this.baseDictionarySnapshot(id);
    const searchRuntime = searchSettingsQueryOptions(baseDictionary.settings?.search);
    ...
    if (canUseSqlStaticTextEntryQuery(normalizedQuery)) {
      return this.queryEntriesStaticTextSql(id, normalizedQuery);
    }
  ...
  async deleteEntry(id, entryId) {
    ...
    try {
      ...
      db.prepare(`
        ...
        WHERE entry_morphology_group_id IN (
          SELECT id FROM entry_morphology_groups WHERE entry_id = ?
        )
      `).run(entryId);
  ...
  writeEntryProjection(db, entry, position) {
    ...
    const insertMorphologyGroup = db.prepare(`
      INSERT INTO entry_morphology_groups(id, entry_id, position, template_group_id, title, notes, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const insertMorphologyOverride = db.prepare(`
      INSERT INTO entry_morphology_cell_overrides(entry_morphology_group_id, template_table_id, row_index, column_index, value)
      VALUES (?, ?, ?, ?, ?)
    `);
    normalizeEntryMorphologyState(entry).morphologyGroups.forEach((group, groupIndex) => {
      ...
      morphologyOverrideRows(group.overrides).forEach((override) => {
        insertMorphologyOverride.run(group.id, override.tableId, override.rowIndex, override.columnIndex, override.value);
      });
    });
  }
...
CREATE INDEX IF NOT EXISTS idx_entry_morphology_overrides_group_id ON entry_morphology_cell_overrides(entry_morphology_group_id);
`;
