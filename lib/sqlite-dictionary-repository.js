...
class SqliteDictionaryRepository {
  ...
  queryEntriesSql(id, query) {
    ...
    const page = rows.map((row) => parseJson(row.entryJson, null)).filter(Boolean);
  ...
  directDerivedEntries(db, sourceEntry, { sort = "lemmaAsc" } = {}) {
    ...
    return db.prepare(`
      SELECT DISTINCT e.entry_json AS entryJson
      FROM entries e
  ...
  rootGroupsFromDatabase(db, { sort = "lemmaAsc" } = {}) {
    const rows = db.prepare(`
      ...
      ORDER BY ${sqlOrderClause(sort, "e")}
    `).all();
    const entries = rows.map((row) => parseJson(row.entryJson, null)).filter(Boolean);
    const groups = new Map();
