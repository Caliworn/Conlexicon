...
class SqliteDictionaryRepository {
  ...
  ipaDistributionFeatureInput(id) {
    const db = this.openDictionaryDatabase(id);
    return {
      entryTotal: Number(db.prepare("SELECT COUNT(*) AS count FROM entries").get()?.count || 0),
  ...
  entrySearchSummary(query, rows, matchedEntryCount) {
    ...
    querySearchFields(query).forEach((field) => {
      ...
      counts.set(field, rows.reduce((count, row) => count + ((row.fieldMask & bit) ? 1 : 0), 0));
    });
  ...
  entryTagStatsFromDatabase(db, dictionary) {
    const configuredParts = [...new Set(
      (dictionary.settings?.partOfSpeechTags || []).map(normalizeStructuralKey).filter(Boolean),
    )];
    const whereClause = configuredParts.length
      ? `WHERE tag NOT IN (${sqlPlaceholders(configuredParts)})`
      : "";
    return {
      tags: db.prepare(`
        SELECT tag, COUNT(*) AS count
        ...
        ${whereClause}
        GROUP BY tag
      `).all(...configuredParts).map((row) => ({
        tag: row.tag,
        displayLabel: displayTag(row.tag, dictionary),
  ...
  analysisActivityStatsFromDatabase(db, limit) {
    const dateBuckets = (column) => {
      ...
      return rows.reverse().map((row) => ({ day: row.day, count: Number(row.count || 0) }));
    };
    return {
  ...
  async queryAnalysis(id, source = {}) {
    ...
    const dictionary = plan.tasks.some((task) => task === "partStats" || task === "tagStats")
