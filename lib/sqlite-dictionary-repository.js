...
function searchProjectionSettingsKey(settings = {}) {
  return JSON.stringify({
    normalization: searchSettingsQueryOptions(settings.search).normalization,
...
class SqliteDictionaryRepository {
  ...
  async updateSettings(id, settings) {
    ...
    const rebuildSearchProjection = searchProjectionSettingsKey(baseDictionary.settings)
      !== searchProjectionSettingsKey(normalized.settings);
    db.exec("BEGIN IMMEDIATE");
    try {
      this.writeModuleBlob(db, "settings", normalized.settings, now);
      if (rebuildSearchProjection) {
    ...
    return {
      clause: `EXISTS (
        ...
        WHERE search_value.entry_id = e.id
          AND search_value.field IN (${sqlPlaceholders(fields)})
          AND instr(search_value.normalized_value, ?) > 0
      )`,
      params: [...fields, query.q],
    };
  }
  ...
  entrySearchHitsFromDatabase(db, entryIds, query) {
    ...
    chunkArray(entryIds).forEach((entryIdChunk) => {
      ...
      `).all(...entryIdChunk, ...fields, query.q).forEach((row) => {
        if (!hitsByEntry.has(row.entryId)) {
          hitsByEntry.set(row.entryId, []);
        }
        hitsByEntry.get(row.entryId).push({
          field: row.field,
          value: row.value || "",
  ...
  async patchEntries(id, updates = [], options = {}) {
    ...
    const rebuildSearchProjection = hasSettings
      && searchProjectionSettingsKey(baseDictionary.settings) !== searchProjectionSettingsKey(nextSettings);
