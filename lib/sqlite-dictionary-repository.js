...
function normalizeRepositoryEntryProbeRequest(request = {}) {
  try {
    return normalizeEntryProbeRequest(request);
  } catch (error) {
    if (error instanceof EntryQueryValidationError) {
      throw apiError(error.message, 400, error.code || "invalid_entry_probe_request", error.details);
    }
    throw error;
  }
}
...
class SqliteDictionaryRepository {
  ...
  entrySearchProjectionMatchSql(query, structural = { clauses: [], params: [] }, options = {}) {
    ...
    if (staticClauses.length) {
      matchQueries.push(`
        ${select} search_value.entry_id AS entry_id
    ...
    if (searchFields.includes("morphology")) {
      ...
      matchQueries.push(`
        ${select} morphology_search_value.entry_id AS entry_id
  ...
  entrySearchMatchedIdsFromProjectionQuery(db, query, options = {}) {
    ...
    const resultSql = options.ordered === false
      ...
      : `SELECT e.id
        ...
        ORDER BY ${sqlOrderClause(query.sort, "e")}`;
    return db.prepare(`
      WITH ${match.candidateCte} matched_ids AS (
        ${match.matchQueries.join(" UNION ")}
      )
      ${resultSql}
    `).all(...match.params).map((row) => row.id);
  }

  entrySearchExistsFromProjectionQuery(db, query, structural) {
    const match = this.entrySearchProjectionMatchSql(query, structural, { distinct: false });
    if (!match.matchQueries.length) {
      return false;
    }
    return Boolean(db.prepare(`
      WITH ${match.candidateCte} matched_ids AS (
        ${match.matchQueries.join(" UNION ALL ")}
      )
      SELECT EXISTS(SELECT 1 FROM matched_ids LIMIT 1) AS available
    `).get(...match.params)?.available);
  }
  ...
  async probeEntryQueries(id, source = {}) {
    const request = normalizeRepositoryEntryProbeRequest(source);
    await this.requireDictionary(id);
    const db = this.openDictionaryDatabase(id);
    const dictionary = this.dictionaryQueryContext(id);
    const searchRuntime = searchSettingsQueryOptions(dictionary.settings?.search);
    ...
    request.queries.forEach((item) => {
      const query = {
        filter: item.filter,
        search: {
          ...item.search,
          rawText: item.search.text,
          text: searchRuntime.normalizeText(item.search.text),
        },
      };
      const identity = JSON.stringify({
        ...
        search: {
          ...
          fuzzyFields: query.search.fuzzyFields,
        },
      });
      let available = availabilityByIdentity.get(identity);
      if (available === undefined) {
        const structural = this.entryQueryWhereClauses(db, dictionary, query);
        if (query.search.text) {
          available = this.entrySearchExistsFromProjectionQuery(db, query, structural);
        } else {
        ...
        availabilityByIdentity.set(identity, available);
      }
      resultEntries.push([item.id, { available }]);
    });

    return {
      dictionaryId: id,
      generation: this.querySessionCache.generation(id),
  ...
  entryPartStatsFromDatabase(db, dictionary) {
    ...
    return {
      parts: [...partMap.entries()]
        .map(([part, count]) => ({ part, displayLabel: displayTag(part, dictionary), count })),
      noPartOfSpeechCount,
    };
  }
  ...
  analysisActivityStatsFromDatabase(db, limit) {
    const dateBuckets = (column) => db.prepare(`
      SELECT STRFTIME('%Y-%m-%d', ${column}) AS day, COUNT(*) AS count
      FROM entries
      WHERE STRFTIME('%Y-%m-%d', ${column}) IS NOT NULL
      GROUP BY day
      ORDER BY day DESC
      LIMIT ?
    `).all(limit).reverse().map((row) => ({ day: row.day, count: Number(row.count || 0) }));
    return {
      created: dateBuckets("created_at"),
      updated: dateBuckets("updated_at"),
      latest: db.prepare(`
        SELECT id AS entryId, lemma, STRFTIME('%Y-%m-%d', updated_at) AS day
        FROM entries
        WHERE STRFTIME('%Y-%m-%d', updated_at) IS NOT NULL
  ...
  async getEntryFacets(id) {
    ...
    const db = this.openDictionaryDatabase(id);
    const dictionary = this.baseDictionarySnapshot(id);
    const partStats = this.entryPartStatsFromDatabase(db, dictionary);
    ...
    const partSet = new Set(partStats.parts.map((part) => normalizeStructuralKey(part.part)));
    const parts = [...partStats.parts]
      .sort((a, b) => a.part.localeCompare(b.part, "zh-CN"))
      .map((part) => ({ tag: part.part, displayLabel: part.displayLabel, count: part.count }));
    return {
      parts,
      tags: tagFacetItems(tagMap, dictionary, partSet),
      noPartOfSpeechCount: partStats.noPartOfSpeechCount,
    };
  }
