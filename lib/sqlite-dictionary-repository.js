...
class SqliteDictionaryRepository {
  ...
  queryEntriesSearchProjectionSql(id, query) {
    const db = this.openDictionaryDatabase(id);
    const dictionary = this.baseDictionarySnapshot(id);
  ...
  queryEntriesFuzzyProjectionSql(id, query) {
    const db = this.openDictionaryDatabase(id);
    const dictionary = this.baseDictionarySnapshot(id);
    const structural = this.entryQueryWhereClauses(db, dictionary, query);
    ...
    const matchedRows = eligibleRows.filter((row) => matchedIds.has(row.id));
    const pageRows = matchedRows.slice(query.offset, query.offset + query.limit);
    const page = query.include === "full"
    ...
    const hasMore = nextOffset < matchedRows.length;
    return {
      items,
      pageInfo: {
        nextCursor: hasMore ? encodeCursor(nextOffset) : "",
  ...
  async queryEntries(id, query = null) {
    ...
    if (canUseSqlSearchProjectionQuery(normalizedQuery)) {
      return this.queryEntriesSearchProjectionSql(id, normalizedQuery);
    }
    return this.queryEntriesFuzzyProjectionSql(id, normalizedQuery);
  }
