...
class SqliteDictionaryRepository {
  ...
  orderedAnalysisFeatureEntryIds(id, candidateIds, query) {
    ...
    const orderedIds = querySearchText(query)
      ? this.entrySearchMatchedIdsFromProjectionQuery(db, query, { ordered: true })
      : db.prepare(`
          SELECT e.id
          FROM entries e
          ORDER BY ${sqlOrderClause(query.sort, "e")}
        `).all().map((row) => row.id);
    return orderedIds.filter((entryId) => candidates.has(entryId));
  }
