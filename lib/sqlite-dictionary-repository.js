...
function canUseSqlStaticTextEntryQuery(query) {
  ...
  // A query without whitespace also cannot depend on the separator inserted
  // between multiple values of one search field by the shared matcher.
  return Boolean(
    query.q
    && /^[\x21-\x7E]+$/.test(query.q)
    ...
    if (query.searchFields.has("tags")) {
      const tagClauses = [`EXISTS (
        ...
        WHERE raw_tags.entry_id = e.id
          AND instr(raw_tags.normalized_tag, ?) > 0
      )`];
      params.push(query.q);
      ...
      if (displayMatchedTags.length) {
        tagClauses.push(`EXISTS (
          ...
          WHERE display_tags.entry_id = e.id
            AND display_tags.normalized_tag IN (${sqlPlaceholders(displayMatchedTags)})
        )`);
