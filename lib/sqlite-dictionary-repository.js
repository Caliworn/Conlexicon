...
function normalizeMorphologyTemplateCellMap(table = {}, rows = 1, columns = 1) {
  const cells = {};
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      const key = `${row},${column}`;
      cells[key] = { sourceText: morphologyCellSourceText(table.cells?.[key]) };
    }
  }
  return cells;
}

function normalizeMorphologyTemplateTable(table = {}, groupId = "", position = 0, fallbackTitle = "") {
  ...
  return {
    ...
    cells: normalizeMorphologyTemplateCellMap(table, rowCount, columnCount),
    createdAt: String(table.createdAt || table.created_at || ""),
    updatedAt: String(table.updatedAt || table.updated_at || ""),
  };
}

function normalizeMorphologyTemplateGroups(morphology = {}) {
  const explicitGroups = Array.isArray(morphology.templateGroups) ? morphology.templateGroups : [];
  if (explicitGroups.length) {
    return explicitGroups.map((group, groupIndex) => {
      const id = String(group.id || `morph-${deterministicUuid(`template-group:${groupIndex}`)}`);
      const tables = Array.isArray(group.tables) && group.tables.length
        ? group.tables
        : [group.table || {}];
      return {
        id,
        position: groupIndex,
        name: String(group.name || group.title || `Morphology Group ${groupIndex + 1}`),
        matchTags: splitQueryList(Array.isArray(group.matchTags) ? group.matchTags.join(",") : group.matchTags || ""),
        notes: String(group.notes || group.note || ""),
        tables: tables.map((table, tableIndex) => normalizeMorphologyTemplateTable(table, id, tableIndex, group.name || group.title)),
  ...
  return (Array.isArray(morphology.tables) ? morphology.tables : []).map((table, index) => {
    ...
    return {
      ...
      tables: [normalizeMorphologyTemplateTable({ ...table, id: morphologyTemplateTableId(id, 0), title: table.title || table.name }, id, 0, table.name)],
      createdAt: String(table.createdAt || table.created_at || ""),
      updatedAt: String(table.updatedAt || table.updated_at || ""),
    };
  });
...
function legacyTablesFromTemplateGroups(groups = []) {
...
function entryMorphologyGroups(entry = {}, tableIdByGroup = new Map()) {
  const explicitGroups = Array.isArray(entry.morphologyGroups)
    ? entry.morphologyGroups
    : (Array.isArray(entry.morphologyTables) ? entry.morphologyTables : []);
  const sources = explicitGroups.length ? explicitGroups : [entry.morphology || {}];
  return sources
...
class SqliteDictionaryRepository {
  ...
  entriesFromRows(db, rows = []) {
    ...
    collectRows((placeholders) => `
      SELECT id, entry_id AS entryId, position, template_group_id AS templateGroupId, title, created_at AS createdAt, updated_at AS updatedAt
    ...
    return rows.map((row) => {
      ...
      const firstOverrides = flattenMorphologyOverrides(firstMorphology.overrides || {});
      return {
        id: row.id,
        lemma: row.lemma || "",
        ...
        notes: row.notes || "",
        morphology: {
          tableId: firstMorphology.templateGroupId || "auto",
          overrides: firstOverrides,
        },
        morphologyGroups: morphologyInstances.map((group) => ({
          id: group.id,
          templateGroupId: group.templateGroupId || "auto",
          title: group.title || "",
  ...
  morphologyFromDatabase(db, blob = {}) {
    ...
    return {
      ...(blob || {}),
      templateGroups,
      tables: legacyTablesFromTemplateGroups(templateGroups),
    };
  }
  ...
  writeMorphologyProjection(db, morphology = {}, updatedAt = "") {
    ...
    normalizeMorphologyTemplateGroups(morphology).forEach((group, groupIndex) => {
      insertGroup.run(
        group.id,
        groupIndex,
        group.name || "",
  ...
  writeEntryProjection(db, entry, position) {
    ...
    const tableIdByGroup = firstTemplateTableIdByGroup(this.morphologyTemplateGroupsFromDatabase(db));
    const insertMorphologyGroup = db.prepare(`
      INSERT INTO entry_morphology_groups(id, entry_id, position, template_group_id, title, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    const insertMorphologyOverride = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
    entryMorphologyGroups(entry, tableIdByGroup).forEach((group, groupIndex) => {
      insertMorphologyGroup.run(
        group.id,
        entry.id,
        groupIndex,
        group.templateGroupId || "auto",
        group.title || "",
        group.createdAt || entry.createdAt || "",
        group.updatedAt || entry.updatedAt || "",
      );
      group.overrides.forEach((override) => {
