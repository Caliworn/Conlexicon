...
function normalizeRepositoryEntryQuery(query = {}) {
  try {
    const normalized = normalizeEntryQuery(query);
    if (entryFilterHasDeferredConditions(normalized.filter)) {
      throw apiError("Entry filter condition is not available yet", 400, "entry_filter_not_implemented");
    }
    return normalized;
  } catch (error) {
...
function canUseSqlSearchProjectionQuery(query) {
  return Boolean(
    query.search.text
    && !query.search.fuzzyFields.length
  );
}
...
class SqliteDictionaryRepository {
  ...
  entryQueryWhereClauses(db, dictionary, query) {
    ...
    if (filter.sourceText) {
      ...
      params.push(filter.sourceText);
    }

    if (filter.derivedFrom.reference) {
      const source = this.resolveEntryBySourceKey(db, filter.derivedFrom.reference);
      const sourceKeys = source
        ? [source.id, source.lemma].map(normalizeText).filter(Boolean)
        : [filter.derivedFrom.reference];
      clauses.push(`EXISTS (
  ...
  entrySearchProjectionWhereClause(query) {
    const clauses = [];
    const params = [];
    const staticFields = query.search.fields.filter((field) => STATIC_ENTRY_SEARCH_FIELDS.has(field));
    if (staticFields.length) {
      ...
      params.push(...staticFields, query.search.text);
    }
    if (query.search.fields.includes("morphology")) {
      clauses.push(`EXISTS (
  ...
  entrySearchHitsFromDatabase(db, entryIds, query) {
    ...
    chunkArray(entryIds).forEach((entryIdChunk) => {
      if (fields.length) {
        ...
        `).all(...entryIdChunk, ...fields, searchText).forEach((row) => {
          addHit(row.entryId, {
            field: row.field,
            value: row.value || "",
      ...
      if (searchFields.includes("morphology")) {
        db.prepare(`
          ...
          WHERE entry_id IN (${sqlPlaceholders(entryIdChunk)})
            AND instr(normalized_value, ?) > 0
          ORDER BY entry_id ASC, position ASC
        `).all(...entryIdChunk, searchText).forEach((row) => {
          addHit(row.entryId, {
            field: "morphology",
            value: row.value || "",
  ...
  entryFuzzyMatchedIdsFromDatabase(db, entryIds, query) {
    ...
    entryIdScopes.forEach((entryIdChunk) => {
      ...
      if (fuzzyStaticFields.length) {
        ...
        staticParams.push(...fuzzyStaticFields, searchText);
      }
      if (strictStaticFields.length) {
        ...
        staticParams.push(...strictStaticFields, searchText);
      }
      ...
      if (searchFields.includes("morphology")) {
        const morphologyFuzzy = fuzzyFields.includes("morphology");
