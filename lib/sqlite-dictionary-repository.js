...
class SqliteDictionaryRepository {
  ...
  entryQueryWhereClauses(db, dictionary, query) {
    ...
    if (filter.derivedFrom.entryId || filter.derivedFrom.reference) {
      const source = filter.derivedFrom.entryId
        ? db.prepare("SELECT id, lemma FROM entries WHERE id = ?").get(filter.derivedFrom.entryId)
        : this.resolveEntryBySourceKey(db, filter.derivedFrom.reference);
      if (filter.derivedFrom.entryId && !source) {
        clauses.push("0 = 1");
      }
      const sourceKeys = source
        ? [source.id, source.lemma].map(normalizeText).filter(Boolean)
        : [filter.derivedFrom.reference].filter(Boolean);
