...
class SqliteDictionaryRepository {
  ...
  serializeRootGroupEntriesPage(id, rootId, query, dictionary, session) {
    const group = session.groups.find((candidate) => candidate.rootId === rootId);
    if (!group) {
      throw apiError("Root group not found", 404, "root_group_not_found");
    }
    const pageIds = group.derivedIds.slice(query.offset, query.offset + query.limit);
    const db = this.openDictionaryDatabase(id);
    const rows = this.entryRowsByIds(db, pageIds);
    const page = query.include === "full"
      ? this.entriesFromRows(db, rows)
      : this.entrySummariesFromRows(db, rows, dictionary);
    const matchedIds = new Set(group.matchedDerivedIds);
    const nextOffset = query.offset + page.length;
    const hasMore = nextOffset < group.derivedIds.length;
    return {
      ...
      pageInfo: {
        nextCursor: hasMore ? encodeCursor(nextOffset) : "",
        hasMore,
        total: group.derivedIds.length,
      },
    };
  }

  async rootGroupQuerySession(id, query, dictionary) {
    if (canUseSqlRootGroupsQuery(query)) {
      const db = this.openDictionaryDatabase(id);
    ...
    const searchRuntime = searchSettingsQueryOptions(dictionary.settings?.search);
    query.rawQ = query.q;
    query.q = searchRuntime.normalizeText(query.q);
    return this.currentQuerySession({
      kind: "rootGroups",
      dictionaryId: id,
      dictionaryUpdatedAt: dictionary.updatedAt,
      query,
      build: () => {
        const snapshot = this.exportDictionarySnapshot(id);
        const compareEntries = (a, b) => compareEntryValues(a, b, query.sort);
        const groups = rootModeGroups(snapshot, {
          query: query.q,
          normalizeText,
          compareEntries,
          matchesEntry: (entry) => entryMatchesSearchText(entry, snapshot, query.rawQ, {
  ...
  async queryRootGroupEntries(id, rootId, query = {}) {
    ...
    const dictionaryContext = this.dictionaryQueryContext(id);
    const session = await this.rootGroupQuerySession(id, normalizedQuery, dictionaryContext);
    return this.serializeRootGroupEntriesPage(id, rootId, normalizedQuery, dictionaryContext, session);
  }
