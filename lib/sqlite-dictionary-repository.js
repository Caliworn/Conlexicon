...
} = require("./legacy-dictionary-migration");
...
class SqliteDictionaryRepository {
  ...
  morphologyFromDatabase(db, blob = {}) {
    ...
    return {
      ...(blob || {}),
      templateGroups,
      tables: temporaryLegacyTablesFromTemplateGroups(templateGroups),
    };
  }
  ...
  writeEntryProjection(db, entry, position) {
    ...
    const insertMorphologyOverride = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
    normalizeEntryMorphologyGroups(entry.morphologyGroups).forEach((group, groupIndex) => {
