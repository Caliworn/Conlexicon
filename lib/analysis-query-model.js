...
function lexiconSummaryWidget(task, includeActions) {
  const entryCount = Number(task.entryCount || 0);
  const derivedCount = Number(task.sourceEntryCount || 0);
  const rootCount = Math.max(0, entryCount - derivedCount);
  const multiSourceCount = Number(task.multiSourceEntryCount || 0);
  return {
    type: "lexiconSummary",
    entryCount,
    rootCount,
    ...
    ...(includeActions ? {
      ...
      rootAction: entryFilterAction({ presence: [{ field: "source", present: false }] }, rootCount),
      derivedAction: entryFilterAction({ presence: [{ field: "source", present: true }] }, derivedCount),
      multiSourceAction: entryFilterAction({ sourceCount: { min: 2 } }, multiSourceCount),
    } : {}),
  };
}
...
function partDistributionWidget(task, widget, includeActions) {
  ...
  if (task.noPartOfSpeechCount > 0) {
    rows.push({ part: NO_PART_FILTER_VALUE, displayLabel: "", count: noPartOfSpeechCount });
  }
...
function tagFrequencyWidget(task, widget, includeActions) {
  ...
  return {
    ...
    rows: (widget.limit === undefined ? rows : rows.slice(0, widget.limit)).map((row) => ({
      ...
      count: Number(row.count || 0),
      ...(includeActions ? {
        action: entryFilterAction({ tags: { values: [row.tag], mode: "any" } }, row.count),
      } : {}),
    })),
  };
}
