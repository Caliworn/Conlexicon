...
const ANALYSIS_WIDGET_DEFINITIONS = {
  ...
  orthographyDistribution: { tasks: ["orthographyStats"] },
...
function partDistributionWidget(task, widget, includeActions) {
  ...
  if (task.noPartOfSpeechCount > 0) {
    rows.push({
      part: NO_PART_FILTER_VALUE,
      displayLabel: "",
      displayAmbiguous: false,
      entryCount: noPartOfSpeechCount,
    });
  }
  ...
  return {
    ...
    rows: (widget.limit === undefined ? rows : rows.slice(0, widget.limit)).map((row) => ({
      ...
      displayAmbiguous: Boolean(row.displayAmbiguous),
...
function tagSetDistributionWidget(task, widget, includeActions) {
  ...
  const taggedEntryCount = Number(task.taggedEntryCount || 0);
  const untaggedEntryCount = Number(task.untaggedEntryCount || 0);
  const multiTagEntryCount = Number(task.multiTagEntryCount || 0);
  return {
    ...
    multiTagEntryCount,
    ...(includeActions ? {
      taggedAction: entryFilterAction(
        { presence: [{ field: "tag", present: true }] },
        taggedEntryCount,
      ),
      untaggedAction: entryFilterAction(
        { presence: [{ field: "tag", present: false }] },
        untaggedEntryCount,
      ),
      multiTagAction: entryFilterAction(
        { tagCount: { min: 2 } },
        multiTagEntryCount,
      ),
    } : {}),
    rows: (widget.limit === undefined ? rows : rows.slice(0, widget.limit)).map((row) => {
      const tags = (row.tags || []).map((tag) => ({
        ...
        displayAmbiguous: Boolean(tag.displayAmbiguous),
...
function orthographyRows(rows, category, includeActions) {
  return (rows || []).map((row) => ({
    value: String(row.value ?? ""),
    entryCount: Number(row.entryCount || 0),
    ...(row.occurrenceCount === undefined ? {} : {
      occurrenceCount: Number(row.occurrenceCount || 0),
    }),
    ...(includeActions ? {
      action: entryFilterAction({
        orthography: { category, value: String(row.value ?? "") },
      }, row.entryCount),
    } : {}),
  }));
}

function orthographyDistributionWidget(task, includeActions) {
  return {
    type: "orthographyDistribution",
    lemmaEntryCount: Number(task.lemmaEntryCount || 0),
    wordLengths: orthographyRows(task.wordLengths, "length", includeActions),
    initials: orthographyRows(task.initials, "initial", includeActions),
    characters: orthographyRows(task.characters, "character", includeActions),
    bigrams: orthographyRows(task.bigrams, "bigram", includeActions),
  };
}
...
function buildAnalysisWidgets(query, taskResults) {
  ...
  return Object.fromEntries(query.widgets.map((widget) => {
    ...
    if (widget.type === "orthographyDistribution") {
      return [widget.id, orthographyDistributionWidget(taskResults.orthographyStats || {}, includeActions)];
    }
