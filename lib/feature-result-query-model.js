...
const FEATURE_RESULT_SOURCE_VERSIONS = new Map([
  [FEATURE_RESULT_SOURCE_TYPE, FEATURE_RESULT_SOURCE_VERSION],
  [IPA_DISTRIBUTION_SOURCE_TYPE, IPA_DISTRIBUTION_SOURCE_VERSION],
]);
...
function normalizeFeatureResultSource(value) {
  ...
  if (!FEATURE_RESULT_SOURCE_VERSIONS.has(type)) {
  ...
  if (version !== FEATURE_RESULT_SOURCE_VERSIONS.get(type)) {
    throw new FeatureResultQueryValidationError(
...
function normalizeFeatureResultView(source, view) {
  if (source.type === IPA_DISTRIBUTION_SOURCE_TYPE) {
