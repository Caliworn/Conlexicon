...
function entryMatchesQuery(entry, dictionary, query) {
  ...
  if (query.derivedFrom) {
    const target = resolveSourceEntry(query.derivedFrom, dictionary, { normalizeText });
    const isDerived = target
      ? entryIsDirectlyDerivedFrom(entry, target, { normalizeText })
      : sources.includes(query.derivedFrom);
    if (!isDerived) {
      return false;
    }
  }
