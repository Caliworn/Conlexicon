...
class JsonDictionaryRepository {
  ...
  async updateSettings(id, settings) {
    await this.requireDictionary(id);
    const dictionary = await this.readDictionary(id);
    return this.writeDictionary({ ...dictionary, settings, updatedAt: new Date().toISOString() });
  }
  ...
  async saveDocs(id, docs) {
    await this.requireDictionary(id);
    const dictionary = await this.readDictionary(id);
    return this.writeDictionary({ ...dictionary, docs, updatedAt: new Date().toISOString() });
  }
  ...
  async saveEntry(id, entry) {
    ...
    const savedEntry = { ...entry, updatedAt: now };
  ...
  async deleteEntry(id, entryId) {
    ...
    return this.writeDictionary({ ...dictionary, updatedAt: new Date().toISOString(), entries });
  }
