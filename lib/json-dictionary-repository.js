...
function assertValidMorphology(morphology) {
  ...
  if (syntaxErrors.length || functionErrors.length) {
    throw httpError("Invalid morphology payload", 400, "invalid_morphology_payload", {
      syntaxErrors,
      functionErrors,
    });
  }
...
class JsonDictionaryRepository {
  ...
  async saveEntry(id, entry) {
    ...
    const savedDictionary = await this.writeDictionary({ ...dictionary, updatedAt: now, entries }, {
      validate: false,
    ...
    return {
      id: savedDictionary.id,
      updatedAt: savedDictionary.updatedAt || now,
      entry: savedDictionary.entries.find((item) => item.id === savedEntry.id),
    };
  }
  ...
  async deleteEntry(id, entryId) {
    ...
    const now = new Date().toISOString();
    const savedDictionary = await this.writeDictionary({ ...dictionary, updatedAt: now, entries }, { validate: false });
    return { id: savedDictionary.id, updatedAt: savedDictionary.updatedAt || now };
  }
