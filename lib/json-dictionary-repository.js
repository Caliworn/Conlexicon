...
function displayTag(tag, dictionary = {}) {
  const value = String(tag || "");
  return dictionary.settings?.tagDisplayMap?.[value] || value;
}

function entryParts(entry = {}, dictionary = {}) {
  ...
  const configuredParts = new Set((settings.partOfSpeechTags || []).map(normalizeText));
  if (!configuredParts.size) {
    return [];
  }
  return tags.filter((tag) => configuredParts.has(normalizeText(tag)));
}
...
function entrySearchText(entry = {}, dictionary = {}) {
  const tags = Array.isArray(entry.tags) ? entry.tags : [];
  return [
    entry.lemma,
    entry.pronunciation,
...
function compareEntryValues(a, b, sort = "lemmaAsc") {
  ...
  const lemmaCompare = collator.compare(a.lemma || "", b.lemma || "") || collator.compare(a.id || "", b.id || "");
...
function entryMatchesQuery(entry, dictionary, query) {
  if (query.q && !entrySearchText(entry, dictionary).includes(query.q)) {
    return false;
  }
  const parts = entryParts(entry, dictionary);
  if (query.part) {
