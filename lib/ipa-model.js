(function initIpaModel(root, factory) {
  ...
  function normalizeIpaOptions(options = {}) {
  ...
  function normalizeIpaSettings(ipa = {}, options = {}) {
    const normalizedOptions = normalizeIpaOptions(options);
    const defaultStress = Number.parseInt(ipa.defaultStress, 10);
    const mappings = [
      ...normalizeIpaRuleList(ipa.mappings, normalizedOptions),
      ...normalizeIpaRuleList(ipa.stressMappings, normalizedOptions).map((rule) => ({
        ...rule,
  ...
  function normalizeIpaRuleList(rules, options = {}) {
    return Array.isArray(rules)
      ? rules.map((rule) => normalizeIpaRule(rule, options)).filter((rule) => rule.from || rule.to || rule.before || rule.after)
      : [];
  }

  function normalizeIpaRule(rule = {}, options = {}) {
    const normalizedOptions = normalizeIpaOptions(options);
