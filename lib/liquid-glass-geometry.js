(function initLiquidGlassGeometry(root, factory) {
  ...
  const REFRACTION_LOOKUP_SAMPLES = 256;
  const OUTER_SHAPES = Object.freeze(["round", "superellipse", "global-superellipse"]);
  ...
  function normalizeSurfaceOptions(options = {}) {
    ...
    const requestedOuterShape = normalizedOuterShape(options.outerShape);
    const outerShape = requestedOuterShape === "global-superellipse"
      ? "global-superellipse"
      : requestedOuterShape === "superellipse" && cornerExponent > 2
        ? "superellipse"
        : "round";
    ...
    const normalTransitionDepth = minimumRadius <= 1
      ? 0
      : Math.min(effectiveBezel, Math.max(guard, Math.min(8, minimumRadius / 2)));
    const normalFieldSoftening = Math.max(guard, effectiveBezel * 0.25);
    ...
    return {
      ...
      effectiveBezel,
      bezelGuard: guard,
      normalTransitionDepth,
      normalFieldSoftening,
      q3Eligible,
      outerShape,
      cornerExponent,
      thickness,
  ...
  function continuousOpticalNormal(x, y, edgeDistance, boundaryNormalX, boundaryNormalY, options) {
    if (
      edgeDistance <= 0
      || edgeDistance >= options.bezel
    ) {
      return { normalX: boundaryNormalX, normalY: boundaryNormalY };
    }

    // A four-side rational potential has full support across the surface. Unlike
    // independently clamped edge influences, it has no activation lines for a
    // wide bezel to expose as diagonal bands. Exact outline normals still own the
    // outer transition so the optical field remains tangent to the CSS shape.
    const softening = options.normalFieldSoftening;
    const left = x + softening;
    const right = options.width - x + softening;
    const top = y + softening;
    const bottom = options.height - y + softening;
    const potentialX = 1 / (right * right) - 1 / (left * left);
    const potentialY = 1 / (bottom * bottom) - 1 / (top * top);
    const potentialLength = Math.hypot(potentialX, potentialY);
    if (potentialLength < 1e-12) {
      return { normalX: boundaryNormalX, normalY: boundaryNormalY };
    }

    const potentialNormalX = potentialX / potentialLength;
    const potentialNormalY = potentialY / potentialLength;
    if (options.normalTransitionDepth <= 0) {
      return { normalX: potentialNormalX, normalY: potentialNormalY };
    }

    const blend = smootherstep(0, options.normalTransitionDepth, edgeDistance);
    const mixedX = boundaryNormalX * (1 - blend) + potentialNormalX * blend;
    const mixedY = boundaryNormalY * (1 - blend) + potentialNormalY * blend;
  ...
  function globalSuperellipseMetrics(x, y, options) {
    const halfWidth = options.width / 2;
    const halfHeight = options.height / 2;
    const px = x - halfWidth;
    const py = y - halfHeight;
    const normalizedX = Math.abs(px) / halfWidth;
    const normalizedY = Math.abs(py) / halfHeight;
    const exponent = options.cornerExponent;
    const implicit = Math.pow(normalizedX, exponent)
      + Math.pow(normalizedY, exponent) - 1;
    const gradientX = exponent * Math.pow(normalizedX, exponent - 1)
      / halfWidth * (px < 0 ? -1 : 1);
    const gradientY = exponent * Math.pow(normalizedY, exponent - 1)
      / halfHeight * (py < 0 ? -1 : 1);
    const gradientLength = Math.hypot(gradientX, gradientY);
    const signedDistance = gradientLength > 1e-9
      ? implicit / gradientLength
      : -Math.min(halfWidth, halfHeight);
    const fallbackLength = Math.hypot(px / halfWidth, py / halfHeight) || 1;
    return {
      inside: implicit <= 0,
      edgeDistance: Math.max(0, -signedDistance),
      normalX: gradientLength > 1e-9
        ? gradientX / gradientLength
        : px / halfWidth / fallbackLength,
      normalY: gradientLength > 1e-9
        ? gradientY / gradientLength
        : py / halfHeight / fallbackLength,
    };
  }
  ...
  function roundedRectMetrics(x, y, options) {
    ...
    const opticalNormal = continuousOpticalNormal(
      x,
      y,
      edgeDistance,
      normalX,
    ...
    return {
      inside: distance <= 0,
      edgeDistance,
      normalX: opticalNormal.normalX,
      normalY: opticalNormal.normalY,
    };
  }

  function surfaceMetrics(x, y, options) {
    return options.outerShape === "global-superellipse"
      ? globalSuperellipseMetrics(x, y, options)
      : roundedRectMetrics(x, y, options);
  }
  ...
  function createRefractionLookup(options, sampleCount = REFRACTION_LOOKUP_SAMPLES) {
    const values = new Float32Array(sampleCount + 1);
    for (let index = 0; index <= sampleCount; index += 1) {
      values[index] = refractionProfile(index / sampleCount, options);
    }
    return values;
  }

  function sampleRefractionLookup(edgeProgress, lookup) {
    const position = clamp(edgeProgress, 0, 1) * (lookup.length - 1);
    const lower = Math.floor(position);
    const upper = Math.min(lookup.length - 1, lower + 1);
    const blend = position - lower;
    return lookup[lower] * (1 - blend) + lookup[upper] * blend;
  }

  function sampleNormalizedSurface(x, y, options, refractionLookup = null) {
    ...
    const metrics = surfaceMetrics(x, y, options);
    const shapeAlpha = options.outerShape === "global-superellipse" && !metrics.inside ? 0 : 255;
    if (!metrics.inside) {
      return {
        displacementX: 0,
        displacementY: 0,
    ...
    if (metrics.edgeDistance >= options.bezel) {
      return {
        displacementX: 0,
        displacementY: 0,
        normalX: 0,
    ...
    const refraction = refractionLookup
      ? sampleRefractionLookup(edgeProgress, refractionLookup)
      : refractionProfile(edgeProgress, options);
  ...
  function sampleSurface(x, y, rawOptions = {}) {
    const sample = sampleNormalizedSurface(x, y, normalizeSurfaceOptions(rawOptions));
    return {
      displacementX: sample.displacementX,
      displacementY: sample.displacementY,
  ...
  function createSurfaceMapGenerator(rawOptions = {}) {
    ...
    const refractionLookup = options.q3Eligible ? createRefractionLookup(options) : null;
    ...
    function step(maxRows = 24) {
      ...
      for (let y = nextRow; y < endRow; y += 1) {
        ...
        for (let x = 0; x < options.mapWidth; x += 1) {
          ...
          const sample = sampleNormalizedSurface(surfaceX, surfaceY, options, refractionLookup);
          ...
          specular[offset + 3] = sample.shapeAlpha;
        }
      }
      nextRow = endRow;
  ...
  function buildCacheKey(rawOptions = {}) {
    ...
    if (options.outerShape === "global-superellipse") {
      parts.push(options.outerShape, options.cornerExponent);
    } else {
      parts.push(...options.radii);
    }
