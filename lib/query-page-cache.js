  ...
  class QueryPageCache {
    ...
    load(options = {}) {
      ...
      const promise = Promise.resolve()
        ...
        .then((value) => {
          if (
            this.dictionaryGeneration(dictionaryId) === generation
    ...
    invalidatePrefix(prefix, options = {}) {
      ...
      let invalidated = 0;
      [...this.entries.entries()].forEach(([key, entry]) => {
        if (key.startsWith(normalizedPrefix) && (!dictionaryId || entry.dictionaryId === dictionaryId)) {
          this.totalBytes -= entry.bytes;
          this.entries.delete(key);
          invalidated += 1;
        }
      ...
      [...this.inFlight.entries()].forEach(([key, pending]) => {
        if (key.startsWith(normalizedPrefix) && (!dictionaryId || pending.dictionaryId === dictionaryId)) {
          this.inFlight.delete(key);
          invalidated += 1;
        }
      });
