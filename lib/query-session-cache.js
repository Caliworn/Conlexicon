const { entryQueryIdentity } = require("./entry-query-model");
...
function createQueryDescriptor({
  ...
  if (baseDescriptor.kind === "entries") {
    const identity = entryQueryIdentity(query);
    return {
      ...baseDescriptor,
      ...identity,
    };
