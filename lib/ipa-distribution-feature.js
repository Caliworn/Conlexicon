...
function incrementFacet(map, value, occurrenceCount = 1) {
  if (value === "" || value === null || value === undefined || occurrenceCount <= 0) {
    return;
  }
...
async function buildIpaDistributionResult(records, ipaSettings = {}, options = {}) {
  ...
  const requestedDictionaryEntryCount = Number(options.dictionaryEntryCount);
  const dictionaryEntryCount = Number.isSafeInteger(requestedDictionaryEntryCount)
    && requestedDictionaryEntryCount >= records.length
    ? requestedDictionaryEntryCount
    : records.length;
  ...
  let unitOccurrenceCount = 0;
  let syllableOccurrenceCount = 0;
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    record.unitCounts.forEach((occurrenceCount, unit) => {
      incrementFacet(facets.units, unit, occurrenceCount);
      unitOccurrenceCount += occurrenceCount;
    });
    ...
    if (record.syllableCount > 0) {
      ...
      syllableOccurrenceCount += record.syllableCount;
  ...
  return {
    ...
    summary: {
      ...
      ipaEntryCount: records.length,
      unitOccurrenceCount,
      syllableEntryCount,
      syllableOccurrenceCount,
      averageSyllablesPerIpaEntry: syllableEntryCount
        ? Number((syllableOccurrenceCount / syllableEntryCount).toFixed(2))
        : 0,
      distributions: {
