(function initEntrySearchModel(root, factory) {
  ...
  function entrySearchValueRecords(entry = {}, dictionary = {}, options = {}) {
    ...
    (entry.etymology?.sources || []).forEach((source, position) => {
      addRecord("etymology", source.text, { sourceType: "etymologySource", sourceId: source.entryId, sourcePosition: position, valueType: "source" });
    });
  ...
  function requireSearchNormalizer(options = {}) {
    if (typeof options.normalizeText !== "function") {
      throw new TypeError("Search normalizeText option is required");
    }
    return options.normalizeText;
  }

  function fuzzyScore(value, query, options = {}) {
    const normalize = requireSearchNormalizer(options);
