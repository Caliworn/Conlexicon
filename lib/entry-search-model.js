(function initEntrySearchModel(root, factory) {
  ...
  function normalizeFuzzyFields(value, options = {}) {
    const source = Array.isArray(value) ? value : splitSearchFieldList(value);
    const fields = source
      .map((field) => String(field || "").trim().toLowerCase())
      .filter((field) => ENTRY_SEARCH_FIELDS.has(field));
    ...
    const fuzzy = booleanOption(options.fuzzy);
    const tagFuzzy = booleanOption(options.tagFuzzy);
    const result = [];
    if (tagFuzzy) {
      result.push("tags");
    }
  ...
  function fieldFuzzyEnabled(field, options = {}) {
    const fuzzyFields = options.fuzzyFields instanceof Set
      ? options.fuzzyFields
      : normalizeFuzzyFields(options.fuzzyFields, options);
  ...
  function textMatches(text, query, options = {}) {
    ...
    return booleanOption(options.fuzzy) && fuzzyScore(text, query, { ...options, normalizeText: normalize }) > 0;
  }
