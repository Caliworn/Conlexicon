(function initEntryRelationsModel(root, factory) {
  ...
  root.ConlexiconEntryRelations = factory(root.ConlexiconSearchNormalization);
})(typeof globalThis !== "undefined" ? globalThis : this, function createEntryRelationsModel(searchNormalization) {
  ...
  function entryIsDirectlyDerivedFrom(entry = {}, sourceEntry = {}, options = {}) {
    return (entry.etymology?.sources || []).some((source) => source.entryId === sourceEntry.id);
  }
  ...
  function rootCount(dictionary = {}, options = {}) {
    const index = options.index || buildEntryRelationIndex(dictionary, options);
    return index.entries.filter((entry) => !entryHasSources(entry)).length;
  }
  ...
  function relationForEntry(entry, dictionary = {}, options = {}) {
    ...
    const sources = sourceTexts.map((source) => {
      const matchedEntry = resolveSourceEntry(source, dictionary, { ...options, index });
      return {
        sourceText: matchedEntry?.lemma || source.text,
        matchedEntry,
