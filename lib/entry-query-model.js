(function initEntryQueryModel(root, factory) {
  ...
  const ENTRY_PROBE_QUERIES_MAX = 16;
  ...
  function normalizeEntryProbeRequest(value = {}) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new EntryQueryValidationError("Invalid entry probe payload", "invalid_entry_probe_payload");
    }
    if (!Array.isArray(value.queries) || !value.queries.length || value.queries.length > ENTRY_PROBE_QUERIES_MAX) {
      throw new EntryQueryValidationError("Entry probe queries are required", "invalid_entry_probe_queries", {
        maxQueries: ENTRY_PROBE_QUERIES_MAX,
      });
    }
    const ids = new Set();
    const queries = value.queries.map((item, index) => {
      if (!item || typeof item !== "object" || Array.isArray(item)) {
        throw new EntryQueryValidationError("Invalid entry probe query", "invalid_entry_probe_query", { index });
      }
      const id = String(item.id || "").trim();
      if (!id || id.length > 80 || !/^[a-z0-9._-]+$/i.test(id)) {
        throw new EntryQueryValidationError("Invalid entry probe query ID", "invalid_entry_probe_query_id", { index, id });
      }
      if (ids.has(id)) {
        throw new EntryQueryValidationError("Duplicate entry probe query ID", "duplicate_entry_probe_query_id", { id });
      }
      ids.add(id);
      const unsupportedKeys = Object.keys(item).filter((key) => !["id", "filter", "search"].includes(key));
      const invalidFilter = item.filter !== undefined
        && (!item.filter || typeof item.filter !== "object" || Array.isArray(item.filter));
      const invalidSearch = item.search !== undefined
        && (!item.search || typeof item.search !== "object" || Array.isArray(item.search));
      if (unsupportedKeys.length || invalidFilter || invalidSearch) {
        throw new EntryQueryValidationError("Invalid entry probe query", "invalid_entry_probe_query", {
          index,
          unsupportedKeys,
        });
      }
      return {
        ...
        search: normalizeEntrySearch(item.search),
      };
    });
    return { queries };
  }
