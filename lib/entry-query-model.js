(function initEntryQueryModel(root, factory) {
  ...
  const ENTRY_QUERY_LIMIT_MAX = 200;
  const ENTRY_FILTER_FACTS_MAX = 16;
  ...
  function normalizeLimit(value, options = {}) {
    const defaultLimit = Math.min(
      ...
      Math.max(1, Number(options.defaultLimit) || ENTRY_QUERY_LIMIT_DEFAULT),
    );
    const maxLimit = Math.min(
      ENTRY_QUERY_LIMIT_MAX,
      Math.max(defaultLimit, Number(options.maxLimit) || ENTRY_QUERY_LIMIT_MAX),
    );
    if (value === undefined || value === null || value === "") {
      return defaultLimit;
    }
    ...
    if (normalized < 1 || normalized > maxLimit) {
      throw new EntryQueryValidationError(
        `Entry query limit must be between 1 and ${maxLimit}`,
        "invalid_entry_query_limit",
        { value, maxLimit },
      );
    }
    return normalized;
  ...
  function normalizeTagCount(value) {
    ...
    const source = typeof value === "number" || typeof value === "string" ? { min: value } : value;
    if (!source || typeof source !== "object" || Array.isArray(source)) {
      throw new EntryQueryValidationError("Invalid tag count filter", "invalid_entry_filter_tag_count");
    }
    if ((source.min === undefined || source.min === null || source.min === "")
      && (source.max === undefined || source.max === null || source.max === "")) {
      return null;
    }
    const min = normalizeNonNegativeInteger(source.min, 0, "invalid_entry_filter_tag_count");
    const max = normalizeNonNegativeInteger(source.max, null, "invalid_entry_filter_tag_count");
    if (max !== null && max < min) {
      throw new EntryQueryValidationError("Tag count maximum is smaller than its minimum", "invalid_entry_filter_tag_count", { min, max });
    }
    return { min, max };
  }
  ...
  function normalizeOrthography(value) {
    try {
      return orthographyModel.normalizedOrthographyFilter(value);
    } catch (error) {
      throw new EntryQueryValidationError(
        "Invalid entry orthography filter",
        "invalid_entry_filter_orthography",
        { cause: error.message },
      );
    }
  }
  ...
  function normalizeEntryFilterFactsRequest(value = {}) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new EntryQueryValidationError("Invalid entry filter facts payload", "invalid_entry_filter_facts_payload");
    }
    if (!Array.isArray(value.filters) || !value.filters.length || value.filters.length > ENTRY_FILTER_FACTS_MAX) {
      throw new EntryQueryValidationError("Entry filter facts are required", "invalid_entry_filter_facts", {
        maxFilters: ENTRY_FILTER_FACTS_MAX,
      });
    }
    const ids = new Set();
    const filters = value.filters.map((item, index) => {
      if (!item || typeof item !== "object" || Array.isArray(item)) {
        throw new EntryQueryValidationError("Invalid entry filter fact", "invalid_entry_filter_fact", { index });
      }
      const id = String(item.id || "").trim();
      if (!id || id.length > 80 || !/^[a-z0-9._-]+$/i.test(id)) {
        throw new EntryQueryValidationError("Invalid entry filter fact ID", "invalid_entry_filter_fact_id", { index, id });
      }
      if (ids.has(id)) {
        throw new EntryQueryValidationError("Duplicate entry filter fact ID", "duplicate_entry_filter_fact_id", { id });
      }
      ids.add(id);
      const unsupportedKeys = Object.keys(item).filter((key) => !["id", "filter"].includes(key));
      const invalidFilter = item.filter !== undefined
        && (!item.filter || typeof item.filter !== "object" || Array.isArray(item.filter));
      if (unsupportedKeys.length || invalidFilter) {
        throw new EntryQueryValidationError("Invalid entry filter fact", "invalid_entry_filter_fact", {
          index,
          unsupportedKeys,
        });
      }
      return {
        id,
        filter: normalizeEntryFilter(item.filter),
      };
    });
    return { filters };
  }
