(function initEntryQueryModel(root, factory) {
  ...
  function normalizeEntryQuery(value = {}, options = {}) {
    ...
    return {
      ...
      include: source.include === "full" ? "full" : "summary",
      page: {
  ...
  function entryFilterHasDeferredConditions(filter = {}) {
    return Boolean(
      filter.presence?.length
      || filter.sourceCount
      || filter.activityDays?.length
      || filter.derivedFrom?.entryId
    );
  }
