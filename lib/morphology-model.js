(function initMorphologyModel(root, factory) {
  ...
  root.ConlexiconMorphology = factory(root.ConlexiconSearchNormalization, root.ConlexiconTags);
})(typeof globalThis !== "undefined" ? globalThis : this, function createMorphologyModel(searchNormalization, tagModel) {
  ...
  function normalizeExistingEntityId(value, usedIds = new Set()) {
  ...
  function normalizeMorphologyTemplateGroup(group = {}, position = 0, options = {}) {
    ...
    const id = normalizedOptions.reserveEntityId
      ? normalizedOptions.reserveEntityId(group.id, "morph", usedIds)
      : normalizeExistingEntityId(group.id, usedIds);
    const defaultName = normalizedOptions.defaultGroupName || "Morphology Group";
    return {
      id,
      name: String(group.name || defaultName),
      matchTags: tagModel.normalizeTagList(group.matchTags),
  ...
  function normalizeMorphologyTemplateTable(table = {}, position = 0, options = {}) {
    ...
    return {
      id: normalizedOptions.reserveEntityId
        ? normalizedOptions.reserveEntityId(table.id, "mtable", usedIds)
        : normalizeExistingEntityId(table.id, usedIds),
  ...
  // This is the canonical entry-side morphology shape used by the
