(function initMorphologyModel(root, factory) {
  ...
  function normalizeMorphologyMode(value) {
    return ["auto", "manual", "none"].includes(value) ? value : "auto";
  }
  ...
  // auto/manual/none model. It intentionally does not interpret the old
  // "auto" and "none" pseudo group IDs; legacy import owns that conversion.
  function normalizeEntryMorphologyState(entry = {}, options = {}) {
    const morphologyMode = normalizeMorphologyMode(entry.morphologyMode);
    const groups = normalizeEntryMorphologyGroups(entry.morphologyGroups, options)
      .filter((group) => isActualTemplateGroupId(group.templateGroupId));

    if (morphologyMode === "none") {
      return { morphologyMode, morphologyGroups: [] };
    }
  ...
  function validateCanonicalEntryMorphology(entry = {}, dictionary = {}, options = {}) {
    const errors = [];
    ...
    if (mode === "none" && sourceGroups.length) {
      errors.push("none morphology mode cannot contain morphology groups");
      return errors;
    }
