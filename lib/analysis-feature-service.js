...
class AnalysisFeatureService {
  ...
  sessionDescriptor(dictionary, generation, source) {
    const ipaSettings = source.type === IPA_DISTRIBUTION_SOURCE_TYPE
      ? normalizeIpaDistributionSettings(dictionary.settings?.ipa)
      : normalizeIpaSettings(dictionary.settings?.ipa);
    return {
      dictionaryId: dictionary.id,
      generation,
      source,
      ...(source.type === IPA_DISTRIBUTION_SOURCE_TYPE ? {} : {
        engine: {
  ...
  async buildSession(dictionary, descriptor) {
    if (descriptor.source.type === IPA_DISTRIBUTION_SOURCE_TYPE) {
      return this.buildIpaDistributionSession(dictionary, descriptor);
    }
    return this.buildIpaAutoCompareSession(dictionary, descriptor);
  }
  ...
  orderedViewIds(dictionary, session, query) {
    ...
    if (query.source.type === IPA_DISTRIBUTION_SOURCE_TYPE) {
      session.recordsById.forEach((record, entryId) => {
        if (ipaDistributionRecordMatches(record, query.view.category, query.view.value)) {
  ...
  responseItems(dictionary, session, orderedIds, query, offset) {
    ...
    return entries.map((entry) => {
      ...
      return {
        entry,
        feature: query.source.type === IPA_DISTRIBUTION_SOURCE_TYPE
          ? ipaDistributionItemFeature(record, query.view.category, query.view.value)
          : record,
      };
    });
  }
