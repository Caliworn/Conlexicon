(function initDictionaryQueryModel(root, factory) {
  ...
  root.ConlexiconDictionaryQuery = factory(
    root.ConlexiconEntryRelations,
    root.ConlexiconSearchNormalization,
  );
})(typeof globalThis !== "undefined" ? globalThis : this, function createDictionaryQueryModel(entryRelationsModel, searchNormalization) {
  ...
  function createDictionaryQueryContext(dictionary = {}, options = {}) {
    ...
    function relationMetrics() {
      ...
      const rootEntries = indexedEntries.filter((entry) => !entryHasSources(entry));
    ...
    function relationSummary() {
      const { rootGroups, rootEntries, derivedEntries, multiSourceEntries } = relationMetrics();
