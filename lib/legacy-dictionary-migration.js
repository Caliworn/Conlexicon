...
const { normalizeText } = require("./search-normalization-model");
...
function normalizeEntryMorphologyGroup(entry = {}, instance = {}, tableIdByGroup = new Map()) {
...
function legacyEntryMorphologyGroups(entry = {}, tableIdByGroup = new Map(), report) {
  ...
  return [legacyMorphology]
    .map((instance) => normalizeEntryMorphologyGroup(entry, instance, tableIdByGroup))
...
function findLegacyAutomaticTemplateGroupId(entry = {}, templateGroups = []) {
  const tags = new Set((entry.tags || []).map((tag) => String(tag || "").trim()));
  return templateGroups.find((group) => (group.matchTags || []).some((tag) => tags.has(String(tag || "").trim())))?.id || "";
}
...
function canonicalizeLegacyEntryMorphology(entry = {}, templateGroups = [], tableIdByGroup = new Map(), report) {
  ...
  if (sourceGroups.some((group) => group && Object.hasOwn(group, "id"))) {
    report.repairs.push("legacy entry morphology group IDs removed");
  }
  ...
  // Current auto-mode groups are overlays keyed by real template group IDs.
  // Preserve them verbatim through JSON import instead of treating them as
  // legacy manual selection data or falling through to the old auto shape.
...
function migrateLegacyDictionary(dictionary = {}) {
  ...
  entries.forEach((entry) => { entry.id ||= `entry-${crypto.randomUUID()}`; });
  const byId = new Map(entries.map((entry) => [entry.id, entry]));
  const byText = new Map();
  entries.forEach((entry) => {
    const key = normalizeText(entry.lemma);
    if (!byText.has(key)) byText.set(key, []);
    byText.get(key).push(entry);
  });
  entries.forEach((entry) => {
    if (!entry.etymology) return;
    entry.etymology.sources = (entry.etymology.sources || []).filter((source) => typeof source !== "string" || source.trim()).map((source) => {
      if (typeof source !== "string") return source;
      const text = source.trim();
      const matches = byId.has(text) ? [byId.get(text)] : byText.get(normalizeText(text)) || [];
      if (matches.length === 1) {
        report.repairs.push(`source reference bound: ${entry.id} -> ${matches[0].id}`);
        return { entryId: matches[0].id, text: matches[0].lemma || text };
      }
      report.warnings.push(`source reference ${matches.length ? "ambiguous" : "unresolved"}: ${entry.id}: ${text}`);
      return { entryId: "", text };
    });
    const sourceTargets = new Set();
    entry.etymology.sources = entry.etymology.sources.filter((source) => {
      // Keep malformed items for normal validation rather than hiding them as duplicates.
      if (!source || Array.isArray(source) || typeof source.entryId !== "string" || typeof source.text !== "string") return true;
      const targetId = source.entryId.trim();
      if (!targetId) return true;
      if (sourceTargets.has(targetId)) {
        report.repairs.push(`duplicate source reference removed: ${entry.id} -> ${targetId}`);
        return false;
      }
...
function migrateLegacyEntry(entry = {}, report, tableIdByGroup = new Map(), templateGroups = []) {
  ...
  delete migrated.meaning;
  delete migrated.example;
  if (Array.isArray(migrated.definitions)) {
    let migratedDefinitionNotes = false;
    const definitions = migrated.definitions
      .map((definition) => {
        if (!definition || typeof definition !== "object" || Array.isArray(definition)) {
          return definition;
        }
        if (definition.notes === undefined) {
          return definition;
        }
        const { notes, ...currentDefinition } = definition;
        migratedDefinitionNotes = true;
        return {
          ...currentDefinition,
          note: definition.note === undefined ? String(notes ?? "") : definition.note,
        };
      })
      .filter((definition) => (
        definition?.meaning || definition?.example || definition?.note
      ));
    if (migratedDefinitionNotes) {
      report.repairs.push("definition.notes migrated to definition.note");
    }
    if (definitions.length !== migrated.definitions.length) {
      report.repairs.push("empty legacy definitions removed");
    }
    migrated.definitions = definitions;
  }
  ...
  if (migratedEtymologyDescription || sourceText || sourceEntryId) {
    ...
    if (!Array.isArray(etymology.sources) || !etymology.sources.length) {
      ...
      etymology.sources = etymology.sources.map((item) => typeof item === "string" ? item.trim() : item).filter(Boolean);
    }
    if (sourceEntryId && !etymology.sources.some((source) => source === String(sourceEntryId) || source.entryId === sourceEntryId)) {
      etymology.sources.push(String(sourceEntryId));
      report.repairs.push("entry.etymology.sourceEntryId migrated to etymology.sources");
    }
    migrated.etymology = etymology;
  }
  delete migrated.roots;
  delete migrated.variant;
  if (migrated.etymology && typeof migrated.etymology === "object" && !Array.isArray(migrated.etymology)) {
    delete migrated.etymology.sourceText;
    delete migrated.etymology.source;
    delete migrated.etymology.sourceEntryId;
  }
...
function migrateLegacySettings(settings = {}, report) {
  ...
  if (settings.ipa && typeof settings.ipa === "object" && !Array.isArray(settings.ipa)) {
    ...
    const hadRuleIds = combinedMappings.some((rule) => rule && Object.hasOwn(rule, "id"));
    migrated.ipa = {
      ...settings.ipa,
      mappings: combinedMappings.map((rule = {}) => {
        const { id, ...textRule } = rule;
        return textRule;
      }),
    };
    ...
    if (hadRuleIds) {
      report.repairs.push("legacy IPA mapping IDs removed");
    }
  }
