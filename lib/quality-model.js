(function initQualityModel(root, factory) {
  if (typeof module === "object" && module.exports) {
    module.exports = factory(require("./ipa-model"));
    return;
  }
  root.ConlexiconQuality = factory(root.ConlexiconIpa);
})(typeof globalThis !== "undefined" ? globalThis : this, function createQualityModel(ipaModel) {
  ...
  function sourceCycleForEntry(entry, dictionary = {}, options = {}) {
    ...
    const visit = (current) => {
      ...
      for (const sourceName of current.etymology?.sources || []) {
        const source = resolveSourceEntry(sourceName, dictionary, options);
  ...
  function buildQualityReport(dictionary = {}, options = {}) {
    ...
    entries.forEach((entry) => {
      ...
      (entry.etymology?.sources || []).forEach((sourceName) => {
        if (!resolveSourceEntry(sourceName, dictionary, { normalizeText: normalize })) {
      ...
      const cycle = sourceCycleForEntry(entry, dictionary, { normalizeText: normalize });
      if (cycle.length) {
