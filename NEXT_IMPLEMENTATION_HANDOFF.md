...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 层叠玻璃 LG-1–LG-4C 已完成：`theme-layered-glass.css` 在 `body[data-ui-skin="layered-glass"]` 下覆盖现有明暗 token，`uiSkin` 支持即时切换；LG-4A 降低内容表面的不透明度但继续禁止重复表面逐项 blur。固定双层线性反射经对照后仅保留在单一 `.entry-display` 词条详情外壳；navigation、navigation-drawer、mobile-bar、floating 和 overlay-panel 保持无固定渐变。LG-4C 局部指针响应只用于能够容纳光标移动的搜索/筛选浮层、信息与确认弹窗和词汇网络面板；桌面与移动导航均退出 allowlist，继续依靠稳定底面、边缘和控件状态高光。桌面导航使用不透明 `navigation` 底面且不执行 backdrop blur，移动覆盖导航通过独立 `navigation-drawer` 角色保留有限透明和 blur。词条详情基础信息直接位于外壳，内部栏目保持近实色且无 blur，窄屏进一步提高不透明度并降低 blur；它不参与 LG-4C，也不扩展到词条列表、编辑表单或分析卡片。所有玻璃角色提供 blur 不可用、减少动态、减少透明度和强制高对比降级；触摸输入不启用动态镜面。主背景环境视差试验已回滚；动态折射、形状融合、设备方向响应和滚动适应均不纳入当前计划。具体规范见 `docs/LAYERED_GLASS_SKIN_SPEC.md`。
