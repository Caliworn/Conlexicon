...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 液态玻璃 LG-1–LG-4C 已完成：`theme-liquid-glass.css` 在 `body[data-ui-skin="liquid-glass"]` 下覆盖现有明暗 token，`uiSkin` 支持即时切换；LG-4A 降低内容表面的不透明度但继续禁止逐项 blur，LG-4B 让 navigation、mobile-bar、sticky 和 floating 通过共享 token 使用两层静态线性反射，LG-4C 以单一全局监听器和逐帧合并让连续导航及当前大型浮层移动既有线性镜面层。桌面导航占据独立布局列，现使用不透明 `navigation` 底面且不执行 backdrop blur；移动覆盖导航通过独立 `navigation-drawer` 角色保留有限透明和 blur，两者共享光学层与控件状态。其余功能层保持有界 blur，并提供 blur 不可用、减少动态、减少透明度和强制高对比降级；触摸输入不启用动态镜面。词条、表格、分析卡片和网络节点不参与指针响应，不新增 DOM、径向光斑、持续动画或持久化状态。动态折射、形状融合和设备方向响应不纳入当前方案，滚动适应只保留为 LG-4D 可选后续。具体规范见 `docs/LIQUID_GLASS_SKIN_SPEC.md`。
