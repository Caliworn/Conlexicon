...
- 搜索字段配置已接入词条列表、词根模式、本地筛选、搜索摘要和搜索字段分析；读取 API 现在只接受 `fields` / `fuzzyFields`，旧 `fuzzy` / `tagFuzzy` 参数已删除。`scripts/benchmark-entry-search.js` 可将指定 JSON 词典临时导入 SQLite 后测量搜索模式。2026-07-12 对无形态模板的 10k 生成词典、查询 `body`、21 次交错样本的中位数为：全字段模糊 369ms、静态字段模糊 359ms、静态字段严格 294ms、仅词形严格 195ms。新增 `scripts/generate-morphology-stress-dictionary-10k.js` 后，对其 10k 词典的初测（`body`、3 次交错样本）为：全字段模糊 3578ms、静态字段模糊 362ms、静态字段严格 309ms、仅词形严格 210ms。该压力词典为每个词性配置 3×4 自动形态模板，并为约 1/29 词条加入单元格 override；结果确认动态形态生成是当前搜索主瓶颈。FTS/索引方案必须先解决 fuzzy 召回语义，不能直接替代当前模糊匹配；动态形态应继续作为显式启用的慢路径。
- `/entries` 已新增最窄的直接 SQL 搜索路径：仅当 `q` 是可打印 ASCII 文本、搜索字段不含 `morphology` 且 `fuzzyFields` 为空时，SQLite 直接筛选静态字段并只为当前页读取摘要依赖；`include=full` 也只重建当前页。基础搜索现在逐个独立字段值匹配，因此 SQL 的 `EXISTS` 语义与共享 matcher 一致：不会跨多个释义、标签、来源、例句或备注拼接命中，短语若完整存在于某一个值内仍可直查。非 ASCII/IPA 查询仍走共享 JavaScript normalizer，保证 locale-aware 大小写语义。词根分组尚未使用这条路径，因为其关系分组语义不能由静态字段查询替代。后续应补针对 SQL 路径与共享匹配器的一致性检查，再决定是否以独立的规范化投影支持非 ASCII 直查。
- 该 SQL 路径已在带形态模板的临时 10k 压测词典上运行 `body`、7 轮交错样本：全字段模糊 1847.18ms、静态字段模糊 188ms、静态字段严格 84.69ms、仅词形严格 3.65ms（均为中位数）。此前静态严格约 309ms 的 3 轮初测不应作严格横向比较，但新路径已确认显著避开完整 snapshot 导出、形态重建和全条目 JavaScript 匹配；含形态的 fuzzy 路径仍是首要慢路径。
- `scripts/check-entry-search-consistency.js` 专门验证 `/entries` 的 SQL 静态严格快路径不会调用完整 snapshot，并以共享 JS 匹配器为参照覆盖全部静态字段、标签显示替换、词性/标签/来源/衍生词筛选、分页和 `include=full`。同一脚本还固定当前 Unicode/PUA fallback 语义（扩展拉丁大小写可匹配，PUA 精确匹配；组合/预组、Greek final sigma、`ß/ss` 与 dotted-I 等尚不作 full case folding）。该检查已于 2026-07-13 通过；在定稿 `normalizeSearchText()` 前，新增检索列或扩展非 ASCII SQL 路径必须先更新这组预期。
- `lib/search-normalization-model.js` 已成为前端与当前 SQLite 主路径共用的规范化入口：`normalizeText(value, locale)` 保留自由文本现有的 `trim + locale lower` 基线，`normalizeSearchText()` 用于自由文本搜索，`normalizeStructuralKey()` 则仅去除边缘空白并保留其余原始码位。标签筛选、手动词性识别、红色高亮和自动形态模板匹配现使用后者；当前 schema 定义已移除 `entry_tags.normalized_tag`，测试 SQLite 必须从 JSON 重新导入。开发期不保存 schema 版本或 migration 表；下一个正式 release 再引入带实际升级脚本的版本化 migration 机制。词源关系与 `source_key` 暂保持既有宽松文本匹配，留待未来升级为稳定词条 ID 引用时再统一处理。`app.js` 仍明确传入当前 UI 的 `zh-CN/en-US` 自由文本搜索 locale，故本次不改变自由文本搜索行为。JSON repository 是 legacy/debug 路径，未为该内部重构继续同步更新。后续先决定 NFC 与 Unicode default case folding 的精确设置语义，再只在本模块中实现并为 SQLite 检索投影复用。
