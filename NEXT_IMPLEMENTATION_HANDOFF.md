...
- `/entries` 的 S3.3 直接 SQL 搜索路径已改为查询静态 `entry_search_values`：只要无 fuzzy 且不搜索动态形态，就能承接 ASCII、Unicode、NFC、case folding 和自定义等价规则，并只为当前页读取摘要依赖和 `searchHits`。包含动态形态或 fuzzy 的请求仍使用共享 JavaScript matcher。词根分组搜索也已读取相同的词典级自由文本配置，但关系分组键仍保持既有独立语义。
...
- `scripts/check-entry-search-consistency.js` 验证静态 projection 查询不调用完整 snapshot，并覆盖逐值字段、命中定位、结构筛选、分页、`include=full`、NFC、Unicode case folding 和 PUA 自定义规则；fuzzy 请求仍单独验证共享 fallback。
