...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 液态玻璃 LG-1–LG-3 已完成：`theme-liquid-glass.css` 在 `body[data-ui-skin="liquid-glass"]` 下覆盖现有明暗 token，`uiSkin` 作为独立全局偏好支持即时切换；稳定内容层不逐项模糊，功能层限制明显 blur 数量，并提供 blur 不可用、减少透明度和强制高对比降级。完整验收已覆盖中英文、明暗主题、320/480/768/1024/1440px、主要页面与浮层，并用临时 20k 词典确认虚拟化渲染和 blur 层数保持有界；原生滚动逐帧表现仍保留为发布前人工观察项。动态折射、形状融合和设备方向响应不纳入首版，指针追光只保留为有实测收益后的限定实验。具体规范见 `docs/LIQUID_GLASS_SKIN_SPEC.md`。
