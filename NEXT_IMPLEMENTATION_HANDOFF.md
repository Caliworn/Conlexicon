...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 液态玻璃 LG-1–LG-4B 已完成：`theme-liquid-glass.css` 在 `body[data-ui-skin="liquid-glass"]` 下覆盖现有明暗 token，`uiSkin` 支持即时切换；LG-4A 降低内容表面的不透明度但继续禁止逐项 blur，LG-4B 让 navigation、mobile-bar、sticky 和 floating 通过共享 token 使用两层静态线性反射。功能层保持有界 blur，并提供 blur 不可用、减少透明度和强制高对比降级。LG-4A/B 已复验中英文、明暗主题、320/480/768/1024/1440px、搜索设置、右键菜单、确认 modal 和词汇网络；基础页面 1 个 blur，打开浮层后为 2 个，均无页面横向溢出。LG-3 的临时 20k 虚拟化边界未因 token-only 光学层增加 DOM 或重复内容 blur；原生滚动逐帧表现仍是发布前人工观察项。动态折射、形状融合和设备方向响应不纳入当前方案，指针追光与滚动适应只保留为 LG-4C/D 可选后续。具体规范见 `docs/LIQUID_GLASS_SKIN_SPEC.md`。
