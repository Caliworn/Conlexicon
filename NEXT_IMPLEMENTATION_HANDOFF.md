...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - S0 样式解耦及后续皮肤职责拆分已完成：`theme-tokens.css` 只承载五个跨皮肤共享圆角，`theme-classic.css`、`theme-layered-glass.css` 与 `theme-liquid-glass.css` 分别完整拥有 92 个非共享标准 token，`styles.css` 只保留布局、组件、状态、响应式规则和 token 消费。产品中的标准 `8px` 控件/面板/浮层及 `999px` 圆头已接入对应共享角色；局部小圆角、液态导航专属几何和 Lab 保持独立。无消费者的 warning soft、普通 tooltip muted、旧 dialog overlay 与预留网络标签 filter 已删除。样式契约持续验证加载顺序、完整皮肤所有权、共享圆角消费者和文件责任边界；具体契约见 `docs/STYLE_SKIN_PLAN.md`。
