...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 液态玻璃 LQ-4–LQ-6 已完成，LQ-7 尚未实施：几何/Worker/filter registry/LRU 引擎通过唯一角色清单服务正式表面；位移图按真实尺寸与圆角生成，第二张贴图编码 X/Y 法线和 rim。Q3 分别位移 R/G/B 后重组真实色散，并由所有缓存 filter 共享的光源向量实时合成 specular；光源移动不进入缓存键、不重建贴图。导航、当前词条、菜单、来源建议、右键菜单、modal 和网络面板通过单一 pointer dispatcher 每帧更新局部光源，tooltip 保持无指针跟随但消费相同 RGB/specular 与提交 settle。等待、生成失败或无 URL filter 直接保留普通 blur；减少动态关闭跟随/settle，减少透明度和强制高对比使用实色。下一步 LQ-7 只做全量性能、资源泄漏与设备级验收，不恢复旧双引擎，也不默认扩展 meniscus bridge。完整边界见 `docs/LIQUID_GLASS_SKIN_SPEC.md` 第 13 节。
