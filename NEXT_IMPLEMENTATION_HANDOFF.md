...
- `/entries` 的直接 SQL 搜索路径只承接严格、可打印 ASCII、无 fuzzy、无动态形态、未启用 case folding 或自定义规则的请求；SQLite 逐值 `instr` 筛选静态字段，并只为当前页读取摘要依赖。其余请求在 S3 检索投影完成前使用共享 JavaScript matcher。词根分组搜索也已读取相同的词典级自由文本配置，但关系分组键仍保持既有独立语义。
...
- `scripts/check-entry-search-consistency.js` 验证严格 SQL 快路径不调用完整 snapshot，并覆盖逐值字段、结构筛选、分页和 `include=full`；同时验证启用 NFC、Unicode case folding 和 PUA 自定义规则后会转入共享 fallback 且结果一致。
- 搜索规范化 S2 已接线：`settings.search.normalization` 支持可选 NFC、Unicode 17 default case folding 和 `{ canonical, variants }[]` 自定义规则；默认严格关闭。词条列表、词根模式、搜索摘要/高亮、搜索字段分析和词源自动补全共用缓存的词典级 normalizer。精确高亮带原文范围映射，可处理 `ß → ss`、NFC 和自定义替换造成的长度变化。标签/词性/形态匹配等结构键不套用自由文本配置；词源关系与 `source_key` 继续保持既有匹配，留待稳定 ID 引用升级。S3 再建立 SQLite 规范化检索投影并扩大直查范围；JSON repository 不同步该功能。
