...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - S0 样式解耦已完成：`theme-tokens.css` 独立承载语义颜色、panel/control/inset/floating/sticky/navigation/tooltip/overlay 等材质角色和圆角契约，`styles.css` 只保留布局、组件、状态和响应式规则；旧 token、迁移别名、组件级暗色分支及主题材质字面量均已删除。`check-style-contract.js` 已进入统一检查入口，持续验证加载顺序、token 完整性和文件责任边界。拆分过程中的明暗主题关键计算样式与 S0 前基线一致；迁移后已完成中英文、明暗主题、320/480/768/1024/1440px、筛选浮层、移动抽屉和确认弹窗浏览器验收。具体契约见 `docs/STYLE_SKIN_PLAN.md`。
   - 液态玻璃 LG-1–LG-4C 已完成：`theme-liquid-glass.css` 在 `body[data-ui-skin="liquid-glass"]` 下覆盖现有明暗 token，`uiSkin` 支持即时切换；LG-4A 降低内容表面的不透明度但继续禁止重复表面逐项 blur，LG-4B 让 navigation、mobile-bar、sticky 和 floating 通过共享 token 使用两层静态线性反射，LG-4C 以单一全局监听器和逐帧合并让连续导航及当前大型浮层移动既有线性镜面层。桌面导航占据独立布局列，现使用不透明 `navigation` 底面且不执行 backdrop blur；移动覆盖导航通过独立 `navigation-drawer` 角色保留有限透明和 blur，两者共享光学层与控件状态。词条查看态另有一个受限的聚焦对象例外：唯一 `.entry-display` 使用静态双层反射与有限 blur，基础信息直接位于外壳，内部栏目保持近实色且无 blur，窄屏进一步提高不透明度并降低 blur；它不参与 LG-4C，也不扩展到词条列表、编辑表单或分析卡片。所有玻璃角色提供 blur 不可用、减少动态、减少透明度和强制高对比降级；触摸输入不启用动态镜面。动态折射、形状融合和设备方向响应不纳入当前方案，滚动适应只保留为 LG-4D 可选后续。具体规范见 `docs/LIQUID_GLASS_SKIN_SPEC.md`。
