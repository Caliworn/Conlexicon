...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - F4a 已实装同步、按需的 `POST /api/dictionaries/:id/analysis/query`：`entryCount`、`lexiconSummary`、`coverageBreakdown`、`partDistribution`、`activityPreview` 和 `activityDistribution` 由最小 widget planner 合并为三个 SQLite 聚合任务。前端总览固定为词汇规模、资料覆盖、词性分布和编辑活动四张卡片；“编辑进度”的完整新增/编辑日期也已异步消费 `activityDistribution`，不再扫描前端完整词条。资料覆盖直接在总览展示并进入有/无字段筛选，不再保留重复的覆盖率子页或前端全词典 coverage slice。词根/衍生词基础计数不建立拓扑，来源覆盖不在首屏重复展示，词根家族与孤立词根延后到关系详情。已迁移统计不再读取完整活动词典；F4a 没有建设通用后台任务框架。
