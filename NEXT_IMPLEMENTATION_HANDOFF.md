...
- 触发标签、词性和高级筛选等词条结果入口时，移动端会打开词条列表抽屉，桌面端会展开已收起的词条列表。
...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - S0 样式解耦已完成：`theme-tokens.css` 独立承载语义颜色、panel/control/inset/floating/mobile-bar/navigation/tooltip/overlay 等材质角色和圆角契约，`styles.css` 只保留布局、组件、状态和响应式规则；旧 token、迁移别名、组件级暗色分支及主题材质字面量均已删除。`check-style-contract.js` 已进入统一检查入口，持续验证加载顺序、token 完整性和文件责任边界。拆分过程中的明暗主题关键计算样式与 S0 前基线一致；迁移后已完成中英文、明暗主题、320/480/768/1024/1440px、筛选浮层、移动抽屉和确认弹窗浏览器验收。具体契约见 `docs/STYLE_SKIN_PLAN.md`。
   - 层叠玻璃 LG-1–LG-4C 已完成：`theme-layered-glass.css` 在 `body[data-ui-skin="layered-glass"]` 下覆盖现有明暗 token，`uiSkin` 支持即时切换；导航底部现使用衣服图标和注册表驱动的顶层悬浮选择菜单，按钮显示当前皮肤，桌面展开栏、窄轨与移动抽屉不再依赖二态循环。LG-4A 降低内容表面的不透明度但继续禁止重复表面逐项 blur。固定双层线性反射经对照后仅保留在单一 `.entry-display` 词条详情外壳；navigation、navigation-drawer、mobile-bar、floating 和 overlay-panel 保持无固定渐变。LG-4C 局部指针响应只用于能够容纳光标移动的搜索/筛选浮层、来源建议、右键菜单、皮肤选择菜单、信息与确认弹窗和词汇网络面板。桌面与移动导航均退出 allowlist，并统一使用经典导航基色的不透明底面且关闭 backdrop blur；移动抽屉通过独立遮罩、边缘和阴影表达覆盖关系，两种布局继续共享导航控件状态高光。词条详情基础信息直接位于外壳，内部栏目保持近实色且无 blur，窄屏进一步提高不透明度并降低 blur；它不参与 LG-4C，也不扩展到词条列表、编辑表单或分析卡片。所有玻璃角色提供 blur 不可用、减少动态、减少透明度和强制高对比降级；触摸输入不启用动态镜面。主背景环境视差试验已回滚；动态折射、形状融合、设备方向响应和滚动适应均不纳入当前计划。具体规范见 `docs/LAYERED_GLASS_SKIN_SPEC.md`。
   - 液态玻璃 LQ-1–LQ-3 已完成并收尾：独立运行期、token 材质、scale 6/14 的页面级 soft/strong SVG 折射和完整实色降级保持既定边界；chip list、标签信息和 rich 三种结构化 app tooltip 统一消费 strong 静态折射与青紫边缘但不进入动态 allowlist。小型菜单、来源建议、右键菜单、modal 和大型网络面板现在通过共享文档级 pointer dispatcher 获得每帧最多一次的中心焦散与 72px 近边缘 glint。当前词条、移动顶栏、tooltip、列表卡片、分析卡片、表格行和网络节点不进入动态 allowlist；触摸、减少动态、减少透明度、强制高对比、失焦、resize、页面隐藏和换皮肤会关闭或清理状态。只读 20k 视图在 1440px/320px 初始分别渲染 7/9 张卡片，移动滚动后为 13，卡片无 blur 或指针属性。具体边界与验收见 `docs/LIQUID_GLASS_SKIN_SPEC.md`。
   - 液态玻璃 LQ-4 几何与滤镜引擎已完成，LQ-5–LQ-7 尚未实施：纯几何模块按表面宽高、四角圆角、bezel、厚度与 IOR 生成确定性位移/高光图，专用 Worker 优先使用 OffscreenCanvas，后备 renderer 分块执行；动态 SVG filter registry、引用计数字节 LRU、ResizeObserver 合并、generation token 和停用清理已接入内部引擎，无注册表面时 Worker 懒加载。`app.js` 只增加 appearance 启停适配；无公共 API/偏好变化、无全局 MutationObserver、无虚拟列表扫描。`?liquid-glass-diagnostics=1` 的独立表面已在 Chromium 明暗主题进入 Q3，换经典皮肤后动态 filter/blob/DOM 全部清理。现有产品消费者仍走 LQ-1–LQ-3，下一步 LQ-5 才按 floating、focus、continuous、modal、micro 迁移正式表面。完整架构与验收见 `docs/LIQUID_GLASS_SKIN_SPEC.md` 第 13 节。
