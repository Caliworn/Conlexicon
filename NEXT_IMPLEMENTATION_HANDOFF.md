...
- `/entries` 已新增最窄的直接 SQL 搜索路径：仅当 `q` 是不含空白的 ASCII 文本、搜索字段不含 `morphology` 且 `fuzzyFields` 为空时，SQLite 直接筛选静态字段并只为当前页读取摘要依赖；`include=full` 也只重建当前页。非 ASCII/IPA 或短语查询仍走共享 JavaScript normalizer，保证 locale-aware 大小写与多值字段拼接语义。词根分组尚未使用这条路径，因为其关系分组语义不能由静态字段查询替代。后续应补针对 SQL 路径与共享匹配器的一致性检查，再决定是否以独立的规范化投影支持非 ASCII 直查。
