...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - F4a 与 F4b-0 至 F4b-3 的核心迁移已完成。F4 收尾仍需把当前前端共享的显示身份消歧接到词典级活跃标签身份快照，确保总览与标签详情使用相同碰撞作用域，并在本地服务可用时完成中英文、主题和窄屏 UI 验收；这不需要新增 SQLite schema。随后进入 F5 质量检查 API，不得把 feature result 伪装成 repository 普通 predicate，也不得为已有 topology/summary 能力重复建会话。
