...
4. 若继续阶段 B，优先处理默认 SQLite 后的清债项：
   ...
   - 液态玻璃 LQ-4–LQ-6 已完成，第一优先级光学校正也已收束，LQ-7 尚未实施：几何/Worker/filter registry/LRU 引擎通过唯一角色清单服务正式表面；位移图按真实尺寸与圆角生成，第二张贴图编码 X/Y 法线和 rim。Q3 资源就绪时只消费动态 URL filter，分别位移并重组 R/G/B，方向性表现只来自 SVG 内按默认环境光向量合成的 specular；导航、抽屉、移动栏、当前词条、浮层、结构化 tooltip、modal 与网络面板使用角色级低透明中性 tint，不再叠加 Q1 的大半径 blur。组件假渐变、固定方向色边/明暗边、CSS radial/conic 假光、局部液态指针调度和 settle 生命周期均已删除；引擎内部 `setLightVector()` 只作为后续正确坐标模型的能力保留。产品背景增加两层极弱、固定的交叉纤维折射参照。资源等待、生成失败或无 URL filter 使用 Q1 普通 blur，减少透明度和强制高对比使用 Q0 实色。下一步先单独讨论 Q2 是否必要，再决定动态光源坐标模型与 LQ-7 验收范围；micro 控件和 Q2 当前定义均未改动。完整边界见 `docs/LIQUID_GLASS_SKIN_SPEC.md` 第 13 节。
